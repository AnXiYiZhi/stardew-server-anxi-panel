...
var (
	...
	jsonPattern = regexp.MustCompile(`(?i)("(?:STEAM_PASSWORD|STEAM_REFRESH_TOKEN|STEAM_APP_TICKET|VNC_PASSWORD|SERVER_PASSWORD|SAP_ROLE_AUTH_KEY|SAP_ROLE_PASSWORDS_B64|refresh_token|app_ticket|password|token|secret|session|cookie|authorization|api_key|apikey)"\s*:\s*")([^"]*)(")`)
	// Matches key=value or key: value assignments.
	// Excludes "authorization" — handled separately by bearerPattern.
	assignmentPattern = regexp.MustCompile(`(?i)\b(STEAM_PASSWORD|STEAM_REFRESH_TOKEN|STEAM_APP_TICKET|VNC_PASSWORD|SERVER_PASSWORD|SAP_ROLE_AUTH_KEY|SAP_ROLE_PASSWORDS_B64|refresh_token|app_ticket|password|token|secret|session|cookie|api_key|apikey)\b\s*[:=]\s*([^\s\r\n,;}]+)`)
	...
	sensitiveKeyPattern = regexp.MustCompile(`(?i)(STEAM_PASSWORD|STEAM_REFRESH_TOKEN|STEAM_APP_TICKET|VNC_PASSWORD|SERVER_PASSWORD|SAP_ROLE_AUTH_KEY|SAP_ROLE_PASSWORDS_B64|refresh_token|app_ticket|password|token|secret|session|cookie|api_key|apikey)`)
