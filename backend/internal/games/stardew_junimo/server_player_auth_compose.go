...
var (
	ErrPlayerAuthComposeUnsupported            = errors.New("player authentication Compose layout is unsupported")
	errPlayerAuthComposeEnvironmentUnsupported = fmt.Errorf("%w: server environment syntax is unsupported", ErrPlayerAuthComposeUnsupported)
)
...
// ensureServerPlayerAuthEnvironmentForLifecycle keeps legacy none/global
// instances compatible with valid custom Compose files whose environment uses
// flow syntax. Role mode remains fail-closed because its per-role credentials
// cannot reach the server unless all SAP_* variables are wired explicitly.
func ensureServerPlayerAuthEnvironmentForLifecycle(dataDir string) (bool, string, error) {
	changed, migrationErr := EnsureServerPlayerAuthEnvironment(dataDir)
	if migrationErr == nil {
		return changed, "", nil
	}
	if !errors.Is(migrationErr, errPlayerAuthComposeEnvironmentUnsupported) {
		return false, "", migrationErr
	}

	state, stateErr := readPlayerAuthEnvState(dataDir)
	if stateErr != nil {
		return false, "", fmt.Errorf("%w; inspect current player authentication mode: %v", migrationErr, stateErr)
	}
	if state.Mode == PlayerAuthModeRole {
		return false, "", migrationErr
	}
	warning := fmt.Sprintf(
		"现有 Compose 的 server environment 使用自定义语法，无法自动补齐角色密码变量；当前玩家认证模式为 %s，已保留原文件并继续启动。切换到角色密码模式前请改为 block mapping/list 语法。",
		state.Mode,
	)
...
func addPlayerAuthEnvironmentToServerSection(section, newline string) (string, bool, error) {
	...
	for i, line := range lines {
		...
		if strings.HasPrefix(line.content, "    environment:") {
			return section, false, fmt.Errorf("%w: server environment must use block syntax", errPlayerAuthComposeEnvironmentUnsupported)
		}
	}
	...
	for _, line := range lines[headerIndex+1:] {
		...
		if !ok {
			return section, false, fmt.Errorf("%w: server environment contains an unsupported entry", errPlayerAuthComposeEnvironmentUnsupported)
		}
		if style != "" && entryStyle != style {
			return section, false, fmt.Errorf("%w: server environment mixes mapping and list syntax", errPlayerAuthComposeEnvironmentUnsupported)
		}
		style = entryStyle
		existing[key] = true
	}
