...
func TestEnsureServerPlayerAuthEnvironmentRejectsUnsupportedInlineEnvironmentWithoutChanges(t *testing.T) {
	...
	if !errors.Is(err, ErrPlayerAuthComposeUnsupported) || !errors.Is(err, errPlayerAuthComposeEnvironmentUnsupported) {
		t.Fatalf("unsupported migration error identity = %v", err)
	}
