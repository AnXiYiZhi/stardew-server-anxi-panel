...
- 健康检查、支持包导出、版本信息。
- 面板正式版本检测，以及基于 Docker/Compose 自容器识别的独立 updater；管理员可全程在 Web 面板触发 panel 单服务升级，页面负责阶段展示、预期断线重连和结果恢复。升级包含 SQLite 一致性备份、三项健康验收和自动回滚，Stardew、server、steam-auth 等游戏服务不在升级操作范围内。
