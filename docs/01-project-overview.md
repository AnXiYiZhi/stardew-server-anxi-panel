...
当前正式版为 `v0.4.11`（commit `ef2580d2e58b170b5e5aa0079496f969228dd3f6`）。本版为首次安装增加持久化排他 owner，修复终态安装页被迟到状态或旧日志重新显示为运行中的问题；SMAPI 内置支持 Mod 会在首次建档前校验并原子物化，Steam 二维码登录取消/超时也不会再遗留 Compose one-off 容器。发布、升级/回滚、首次建档、三仓回拉和资源清理证据统一见 `docs/09-image-build.md`。
