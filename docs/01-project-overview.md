...
当前正式版为 `v0.4.14`（commit `a70efc98feecd6b2db803435b59b0f31d1439cf3`）。本版修复 Control 启动状态误判和 error→重装映射，新建档采用持久幂等、单写入者、完整角色内存/同 ID 保存/双 XML 耐久门禁；宿主重启后游戏保持关闭，由用户手动开启。Release workflow、三仓精确版/latest、附件和逐仓启动/重启均已通过；生产同步因新主机 SSH 用户名未确认而待完成。完整证据见 `docs/09-image-build.md`。
