# V060-FINAL-SAFETY-PREFLIGHT-1：授权收尾、凭据并发与启动升级门禁收口（2026-08-27，release preflight）
...
- 以上能力计划进入 `v0.6.0`，但本节不代表已发布。正式候选、`v0.5.13 → v0.6.0`、最低支持边界 `v0.3.2 → v0.6.0`、annotated tag、三仓唯一 digest、`latest`、正式 smoke 与 GitHub Release 证据均待发布门禁完成后回填。

# STEAM-INVITE-STARTUP-WAIT-1：启动期邀请码缺失不再误报 Auth 异常（2026-08-26，local/unreleased）
...
# CABIN-VANILLA-DEFAULT-1：小屋缺省统一为原版（2026-08-26，local/unreleased）
...
# STEAM-INVITE-OPTIN-1：SteamCMD 默认安装与 Steam 邀请码按需启用（2026-08-26，local/unreleased）
