...
- 任何 active job、多个未完成 operation、job/token/journal 身份缺失或冲突、运行时未停、`phaseAFifoWriteAttempted/upstreamSubmitted/upstreamConfirmed`、指针或指纹变化仍 fail closed，返回原有 `save_import_busy/import_recovery_required/save_in_progress`，不会为了消除提示重放 FIFO、删除未知存档或覆盖同名目录。
