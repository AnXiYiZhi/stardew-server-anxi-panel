...
## 2026-09-17：新世界继承首世界 VNC 密码（本地完成，未发布）

- `stardew_junimo/instance_provision.go` 的新建世界流程继承默认安装世界的 `VNC_PASSWORD`；首世界密码为空/纯空白时拒绝创建，`POST /api/instances` 返回 `409 vnc_password_required` 并按既有事务回收目录、卷和 reservation。Steam 凭据、邀请码 session 与端口仍按各自现有边界处理。
- 继承仅发生于创建；既有镜像兼容收敛仍只更新运行身份，保留世界已经设置的密码。`config/env.go` 同步修正双引号值中反斜杠和引号的反转义，使密码读写/重复 env merge 保持一致。
- 验证：driver/Web 创建、空密码拒绝及清理、非管理员拒绝、接口不泄露密码、特殊字符与重复 merge、既有密码保护回归通过；config 全包、Go vet 与 build 通过。本机 `stardew-2` 已经由安全配置写入继承首世界密码，经面板确认 Saved 后重启，运行态密码一致、`SecurityTypes=VncAuth`、容器 healthy。首世界未重启。当前未发布，新建逻辑在后端加载本次构建后生效。
...
# WORLD-PROVISION-REVIEW：创建资源归属、互斥与恢复（2026-09-05，未发布）

- Review 的三项后端问题已修复：创建失败不能删除同名旧卷；复制期间模板不能被维护任务修改；Panel 中断后不能遗留永久 provisioning 记录。新增 migration `016_instance_provisions.sql`，在分配资源前记录源/目标与随机 token，事务拒绝两端或全局活动任务，触发器阻止晚到的任务。driver 的运行栈锁覆盖校验、Prepare、复制、验证和发布；未完成创建还会阻止 Prepare、凭据修改、生命周期与维护恢复。
- Docker 卷与临时 copier 使用同一 token；幂等 volume create 后再次 inspect 归属，复制失败只回收该 token 的资源，旧卷或重建异主卷保持原样。目录先独占发布 `owner.json`，清理检查受管精确子目录、符号链接/挂载与 token。无 journal 时不推断资源归属。
- 成功发布 `save_required/instance_ready` 与删除 journal 为同一 SQLite 事务。启动在其它恢复入口之前回收未完成创建并重载实例；失败保留 journal 与 `instance_provision_cleanup_failed`，管理员可重试删除继续清理。仅有预留行且无目录时自动释放；历史无归属文件保留并提示处理。Web 资源清理未完成会准确返回 `instance_provision_cleanup_failed`。
- 文件：`internal/{storage,docker}/instance_provision*`、`stardew_junimo/instance_provision*`、`instance_delete.go`、`driver.go`、`steam_credentials.go`、`web/instance_handlers.go` 与 `cmd/panel/main.go`。SQLite/driver/Web 定向回归通过；真实 Docker 所有权与中断 copier 回收测试通过（10.51s）。发布验证和剩余运行注意见 `docs/09-image-build.md`。
...
# STEAM-CREDENTIAL-AUTO-GUARD-1：账号密码登录与 Steam Guard 自动分流（2026-09-04，未发布）

- 新发起的基础安装与 Steam 邀请授权统一使用已提交/保存的 Steam 账号密码。`AuthLoginOnly` 也强制复用凭据模式并运行无游戏下载的 `serve`，不会停在登录方式菜单；`ForceReauth` 仍只负责清理失效 session，不改变凭据登录方式。
- SteamAuth 与 SteamCMD 按上游实际提示自动分流：直接要求验证码时进入 `steam_guard_required` / `steamcmd_guard_required` 并等待用户输入；直接要求手机确认时进入对应 mobile phase；若上游显示验证方式菜单，runner 只自动提交一次选项 `1`，进入 Steam 手机 App 批准等待，用户不再选择验证类型。
- 旧任务 phase 与输入 API 继续兼容恢复和取消，既有二维码实现仍只作为历史任务收尾边界；新任务不会进入该路径。主要影响 `stardew_junimo/{driver,installer,driver_test}.go`，专项覆盖 Auth-only 凭据命令、手机批准自动选择、凭据失败清理以及 SteamCMD 菜单自动选择；未用真实账号执行登录或下载。

# LIFECYCLE-RECONCILE-OWNER-1：启动前置阶段不再被只读状态校正打断（2026-09-04，未发布）

- `stardew_junimo.Driver.ReconcileState` 现在先识别仍活跃的实例生命周期任务：实例已经写入 `starting`、但 Compose 尚未创建 `server` 的准备窗口内，读取 `/state` 会保留任务拥有的状态，不再按“容器缺失”改写为 `stopped/container_stopped`。容器出现后的既有 owner 保护、任务终态写入和 Control ready 门禁保持不变。
- 没有活跃生命周期任务的历史 `starting/running` 仍按 Docker 事实收敛，避免真正异常退出被掩盖。`GET /api/jobs` 和 job detail 为 `stardew_lifecycle` 增加可选 `operation`，只从 payload 提取 `start/stop/restart/restore_restart/new_game/new_game_rollback` 白名单值；完整 payload 及其中的新建配置仍不对外返回。数据库、Docker 和任务持久格式不变。
- 回归 `TestDriverReconcileStateDoesNotDemoteActiveLifecycleOwnerBeforeServerExists` 与既有容器已出现 owner 用例一并通过，覆盖启动准备期的持久状态不被读取请求修改；`TestJobResponseExposesOnlySafeLifecycleOperation` 覆盖 stop 方向可恢复、未知/畸形/非 lifecycle payload 不公开且完整 payload 不泄漏。

# REQUIRED-RUNTIME-TRANSIENT-RETRY-1：旧实例镜像恢复后自动续跑必需栈升级（2026-09-01，未发布）

- `StartRequiredRuntimeUpdate` 继续按 Panel version + stack version 抑制确定性失败循环，但把 `current_server_digest_unavailable`、`current_auth_digest_unavailable` 与既有 `context_cancelled` 明确归为可恢复失败。管理员补齐内置清单中精确 digest 的可信本地镜像后，下次 Panel 启动会重新执行既有 dry-run → apply 事务；自定义镜像、不可信配置、回滚失败及其它确定性错误仍保持人工处理，不会被自动覆盖。
- 本机旧默认实例曾因 Auth 当前别名无法解析 canonical digest 停在 `current_auth_digest_unavailable`。从内置候选拉取并核对 `steam-auth-cn 1.5.0-anxi.2` 的 canonical digest 后，重启 Panel 自动完成 required 升级：required/apply 均为 `succeeded`，Control 更新至 `0.3.8` 且 DLL SHA-256 为 `dda16954bf3188222b4c8cdbf8d95f75dd2550748a40ec227a33706cf274d3cf`；实例按升级前状态停回，原 1 个存档目录保留，第二实例容器全程健康运行。此次只操作本机测试数据与镜像缓存，未访问生产数据。
- 新增回归 `TestRequiredRuntimeUpdateResumesAfterCurrentAuthImageBecomesAvailable`。定向重试用例、Windows 排除既有 POSIX mode 专项后的 Junimo 全包（251.997s）、`go vet ./...` 和 `go build ./cmd/panel` 通过；Windows 全包唯一失败仍是已记录的 `TestEnsureInstanceDockerHostBindingsMigratesLegacyCompose` 权限位环境差异。

# MULTI-GAME-INSTALL-BACKEND-1：公用 Steam 下载层与安全世界创建（2026-09-01，未发布）

- 新增 `internal/games/steamcmd` 公用层。一个 Panel 进程只创建一个 `Manager` 并注入游戏 driver；它在 `<PANEL_DATA_DIR>/shared/steam-download/credentials.json` 保存 Steam 下载账号、密码和 SteamCMD 设备授权完成态，目录/临时文件使用 `0700/0600` 并原子替换。API 只返回 `credentialsConfigured` / `authorizationCached`，从不返回用户名或密码。旧实例 `.env` 的凭据仅在公用存储为空时迁移一次，不覆盖后来明确保存的公用账号。
- 公用层统一提供短命 SteamCMD 容器请求构造、跨镜像 HOME/登录目录挂载、Panel 命名空间下的 `steamcmd-login` / `steamcmd-home` 卷，以及进程内下载互斥。游戏 driver 只提供镜像引用、目标卷与 `AppID/installDir/anonymous` 清单；星露谷当前为账户 App `413150` 和匿名 SDK App `1007`。匿名 App 不接收账户密码，密码只进入一次性容器环境且不写任务日志。Docker daemon 的已拉镜像层可自然复用，但镜像候选和运行文件完整性判定仍属于具体游戏 driver。
- `GET /api/games/stardew/installation` 要求已登录，是成员和管理员均可读取的非秘密游戏级安装投影；返回固定 `installationTargetId`、driver、required-files 结果、公用凭据/设备授权是否就绪和兼容实例 DTO。全局安装页继续调用既有实例安装 job；driver 会从公用凭据层解析账号，基础下载 job 在所有实例间互斥，世界级 `stardew_steam_auth` 邀请授权仍只与自己的实例冲突。
- `POST /api/instances` 现提供第一条 driver-owned 创建契约，请求收口为 `{name,gameId:"stardew"}`，仅管理员可用，客户端不能选择内部实例 ID。migration `014_instance_id_sequences.sql` 为每个游戏保存只增不减的下一编号；storage 在首次分配时兼容现有实例数量和 `stardew-N` 最大后缀，此后事务化生成 `stardew-2`、`stardew-3`，创建失败或实例删除也不复用旧编号。Web 层只负责生成 ID/目录边界、数据库 reservation、审计和失败编排；公众不能选择 source driver、Docker volume 或模板。Stardew driver 固定使用默认游戏安装目标作为内部模板，拒绝未安装/活动任务，分配未占用的 game/query UDP 与 VNC/API TCP 端口，生成独立实例目录/Compose，再通过 Docker helper 在 `--network none --pull never` 下把已验证的游戏运行卷只读复制到新的 `<instance>_game-data` 卷。复制后再次验证必需运行文件，成功发布为 `save_required`；存档、Mod、设置和邀请码 session 均不从模板复制。
- 新实例同时继承模板已经验证可用的 `IMAGE_VERSION`、`SERVER_IMAGE`、`SERVER_IMAGE_CANDIDATES` 与 SMAPI 版本/下载元数据，兼容旧版本升级后仍使用镜像代理或自定义可信仓库的默认实例；创建时额外继承首世界 VNC 密码（2026-09-17 修复），不会复制 Steam 账号、Steam 邀请授权或其它实例设置。Panel 启动时还会修正早期多实例实现留下的窄兼容状态：仅限 `save_required/instance_ready`、受管目录和标准独立 game-data 卷，且目标镜像明确缺失、模板镜像本地存在、两者 tag 相同、目标运行文件用模板镜像复验完整时，才把目标运行镜像身份收敛到模板；不下载游戏、不改 game-data 或存档，用户自定义版本/已可用镜像保持不动。
- 创建失败会先按精确实例根、ID 和目标卷清理；清理成功才删除 `instance_provisioning` reservation，清理不完整则保留 error 实例供诊断，不把部分资源伪装成已创建。当前模板仍是默认安装目标的已验证 game-data 卷，不是独立不可变制品；这一边界使现有安装/修复状态机无需复制，也避免先引入未讨论的共享运行容器架构。未来若增加专用不可变模板卷，应由 driver 迁移并补版本/回滚契约，不能由 API handler 拼 Docker 逻辑。
- 主要文件：`backend/internal/games/steamcmd/*`、`stardew_junimo/{driver,installer,shared_steam_download,instance_provision,steam_credentials}.go`、`internal/docker/instance_provision.go`、`internal/storage/instances.go`、`internal/web/{game_installation,instance,install}_handlers.go` 与 `cmd/panel/main.go`。新增模板继承/启动期兼容专项、SteamCMD/Stardew 定向回归、storage/docker/Web 回归、Go vet 和 Panel build 均通过；Windows 全量仅命中既有 POSIX mode 假失败，Linux Junimo 包越过该权限断言后仅命中既有异步测试的 `TempDir` 清理竞态，失败用例精确重跑通过，任务容器和两个缓存卷已清零。未运行真实 Steam 下载、未写生产数据、未发布。

# WEB-MULTI-GAME-SPA-PATHS-1：多游戏入口与动态实例 SPA fallback（2026-08-31，未发布）

- `internal/web/handler.go` 的已知浏览器路径新增 `/games`、`/games/stardew`、`/games/stardew/install` 和 `/games/stardew/new`，并把旧的硬编码 `/instances/stardew/:page` 收紧为 `/instances/:safe-id/:page`。safe ID 只允许单段 ASCII 字母、数字、点、下划线和连字符；page 仍限现有九个 Stardew 页面，未知页、空 ID 与额外嵌套路径不会得到 SPA shell。
- 本次只扩展前端刷新/深链的静态 fallback，没有新增 `POST /api/instances`、实例创建、端口/目录/容器分配或共享 game-data 逻辑。实例目录继续由现有 `GET /api/instances` 提供，driver 仍是唯一 Stardew 运行逻辑边界。
- `handler_paths_test.go` 覆盖新游戏路径、动态 `river-farm` 实例、特殊字符/空 ID/未知页/嵌套路径拒绝；定向 `go test ./internal/web -run 'TestKnownRequestPath' -count=1` 通过。完整跨端路由与已知 diagnostics 默认实例边界见 `docs/06-integration.md`。

# LOGIN-CHAT-PRIVACY-1：服务端丢弃 `!login` 凭据广播（2026-08-28，released in v0.6.1）
...
- proof artifact=`release-candidate-0.6.0-9c6d9c7696c6`（ID `9647693527`），自动 annotated tag workflow=`33075599631`，tag object=`c101344eaed4ff4f3d45d7b8949bdded8d6d69f1`，正式提升 workflow=`33075622114`；三仓 `0.6.0/latest` 六引用统一 digest=`sha256:e9c1613a7ffbd13d92d5a197d751cb5de6b08b65f74351e39a4ad0f9b4598d16`。
- 正式 GHCR smoke、OCI version/revision/created、GitHub Release 与四项部署资产摘要均通过。下面原 preflight/local 章节作为实现演进记录保留，若其旧时态与本节冲突，以本节和 `docs/09-image-build.md` 的不可变发布证据为准。

# V060-FINAL-SAFETY-PREFLIGHT-1：授权收尾、凭据并发与启动升级门禁收口（2026-08-27，released in v0.6.0）

- SteamAuth 观察到真实登录成功后，通过 `SetSteamAuthCompletedState` 在一次 `.env` 原子重写中同时保存 `STEAM_AUTH_COMPLETED=true`、`STEAM_INVITE_ENABLED=true` 与 `ready|cleanup_pending`。如果一次性 Auth holder 停删失败，状态保持 `cleanup_pending`，session volume 和成功证据均保留；下一次启动、`Prepare` 或再次操作只能在确认 server 停止、完成态仍存在且所有 holder 通过安全分类后收敛 holder，再改为 `ready`。未知 holder、缺少成功证据或 Docker 状态不确定均 fail closed，不能启动 server、重复授权或删除 session volume。
- `UpdateSteamCredentials` 现与生命周期、安装和 Auth reservation 共用 `runtimeUpdateMu`。锁内重新读取权威实例，重新校验已安装状态、unfinished new-game owner、active lifecycle/install/Auth job，并通过无缓存 `ComposePsStrict` 证明 server 已停止，之后才只更新 `STEAM_USERNAME/STEAM_PASSWORD`；任何竞态都在写入前返回 409，SteamCMD cache、SteamAuth session、邀请码意图和 game-data 不变。
- Panel 启动时不再只 `Prepare` 默认实例：每个 Stardew 实例都必须先完成 `Prepare`，且没有 runtime/SMAPI recovery 正在执行，才启动 required runtime coordinator。单实例迁移失败只跳过该实例，避免用半迁移 `.env` 进入 server-only 升级；其它实例继续处理。
- `RuntimeServerHealth` 继续对公开 `CommandResult` 做脱敏，但 JSON 判定改读命令 wrapper 内部私有原始 stdout，修复健康响应被 redaction 丢弃后恒判失败的问题；原始 Docker inspect/env 不进入日志或返回值。
- enabled 实例每次真实启动、重启或受控恢复到新一代运行栈时，都会在 `DriverPayload.steam_invite_warmup_started_at` 写入专用 UTC 起点；普通 running 状态更新、邀请码 payload 写入和其它无关更新不得刷新它。`GET .../invite-code` 在 `starting` 或该 marker 起 10 分钟内遇到 Auth/exec 暂时失败时返回 `generating`，窗口到期后返回 `auth_unavailable`；marker 缺失、损坏或位于未来均 fail closed。该边界不改授权完成态、session 或 LAN/IP 可用性。
- 影响 `config/env.go`、`installer.go`、`steam_invite_{runtime_scope,recovery}.go`、`steam_credentials.go`、`docker/runtime_apply.go`、`cmd/panel/runtime_bootstrap.go`、邀请码暖机写入/HTTP 路径及对应测试。暖机补丁后任务专属 Linux Go 1.25.11/DinD 已再次通过 `go test ./...`（Junimo `87.035s`、Web `56.122s`）、`go vet ./...`、`go build -o /tmp/anxi-v060-panel ./cmd/panel`，owner container/network/volume 精确复核为 `0/0/0`；Windows Web 包也通过（`36.324s`），Junimo 包除既有 NTFS/POSIX mode 夹具外为 `100.944s`。最终 `v0.5.13` 与 `v0.3.2` 双升级、候选 proof、tag 和 promotion 已由本文件顶部记录的正式链完成。
- v0.6.0 候选的存档导入失败恢复验收必须与 scoped Stop 保持一致：`RuntimeComposeStopServices` 不删除 container、network 或 volume；安全终态只要求实例 snapshot 恢复、项目无运行容器，且 disabled fixture 最多保留一个 `exited|dead` server 和精确默认 network，绝不能物化 Auth 或留下其它 orphan。单元 maintenance fake 也在 stop 后返回 retained exited server，使 snapshot 恢复测试不再只覆盖旧 ComposeDown 空集合。候选 Shell 对每条 Docker/inspect 管道显式检查失败，不能因 Bash 条件上下文抑制 `errexit` 而把探针失败计成 0。测试资源归零属于随后由 fixture 执行的精确 `down --remove-orphans`，不得重新把“container/network 为 0”当作产品恢复语义。
- 第五轮真实升级预演进一步暴露了产品级 deadline 竞态：存档 maintenance/Phase A 的外层恢复预算原为 30 秒，而 scoped Stop 内部正好执行 `docker compose stop --timeout 30`；需要 SIGKILL 的 PID 1 会让 CLI 在 daemon 已完成停服的边界先报超时，旧实现随即跳过实例 snapshot 恢复。现统一使用 150 秒 save-import 停服预算（严格大于 Docker Down 的 2 分钟命令上限），并在 stop 结果模糊或父 context 到期时用独立的 30 秒 `ComposePsStrict` 复核（严格大于 Ps 的 15 秒命令上限）；只有 disabled 的 server，或 enabled 的 server 与 Auth 全部确证 absent/exited/dead，才恢复原 state、state-message NULL/字节、driver phase/payload 和 journal=`snapshot_restored`。maintenance 的普通错误与 panic 都先执行同一 stop/恢复 defer；Phase A 在 FIFO 提交边界前 panic 会恢复 snapshot，边界后或边界不确定则只停服并写 `manual_required`，不冒险回滚可能已经执行的游戏写入。stop 后仍 running/unknown、Auth 未停或严格探针失败继续保持 recovery-required，且 job 终态只会在这些 defer 完成后发布。
- Maintenance 启动前现在用错误可见的 `SteamInviteEnabledStrict` 读取意图，并把 `maintenanceSteamInviteEnabled` 写入 operation journal；Compose start、所有错误/panic defer、Phase A、activation rollback 与 Panel 重启恢复只使用该冻结范围，不会因 `.env` 后续改值、临时不可读或非法值把已启动的 enabled Auth 静默降成 server-only。旧 journal 无该字段时保守按 server+Auth 停服；journal 丢失/损坏时同样尝试保守停服但禁止 snapshot restore。`snapshot_restore_pending` 也必须再次对冻结服务集执行 scoped Stop 与 strict proof。FIFO 已成功后 journal reload 失败、以及 Phase A 首次 journal 读取失败都进入 `manual_required`/recovery-required 停服路径且绝不重发命令。Save-import defer 使用固定 panic 文案，`jobs.Manager` 的最终 recover 也统一只持久化“任务执行异常。”，因此 preflight 或 cleanup 自身二次 panic 的原始密码、token、平台 ID/路径同样不会写入 job log/error。
- `Prepare` 在读取或恢复任何 save-import journal 前先拒绝同实例仍处于 queued/running 的导入 runner，再从 store 重读权威实例；进入 maintenance 时把 `save_import_operation_id` 一并写入 `DriverPayload`，防止旧参数快照或另一条 completed journal 抢占恢复。owner/payload 损坏、owner 指向错误事务或多个旧 completed journal 令归属无法证明时，会保守停 server+Auth、禁止 snapshot restore；只有找到唯一 completed owner 后的 server 探针或 SQLite 发布失败才保持 committed runtime 原样运行、零 stop，供下次重试。completed journal 是不可逆提交边界：重启恢复只要求 server 严格确认为 running，Auth 仅为邀请码的 advisory sidecar；随后幂等发布实例 `running`、清除 operation owner，并按冻结 enabled 意图写入邀请码暖机 marker；重复发布真正 no-op，不刷新暖机时间，也不能覆盖非 maintenance phase。任一 `RollbackState` 或缺少严格 no-effect 证据的 `manual_required` 都高于 activation resume；唯一例外是已有完整 Phase A no-effect 证据时只停私有 runtime/恢复 snapshot，仍绝不 resume activation。confirmed step panic 只暴露固定错误，swap 在未完成时按 journal 回滚，as-is 停服后保持 manual，completed 后 panic 只重试状态发布。
- 最终 owner/恢复修复已在任务专属 Linux Go 1.25 环境通过 `go test ./... -count=1`（Junimo `208.566s`、Web `55.980s`）、`go vet ./...` 与 `go build -o /tmp/anxi-v060-panel ./cmd/panel`；任务 container 和两个独立 Go cache volume 精确归零。Linux 第一轮据测试发现 strict no-effect maintenance 被过早归为 manual，第二轮发现旧 durable 测试夹具缺真实 owner/phase；修正后第三轮全绿。最终双升级随后已从同一不可变候选完成，权威证据为 run `33073661356`。

# V060-UPGRADE-PREFLIGHT-1：显式意图迁移与 server-only 升级收口（2026-08-26，released in v0.6.0）

- `STEAM_INVITE_ENABLED` 已成为升级后的唯一用户意图。键存在时显式 `true/false` 永远优先；旧实例缺键时只接受强 SteamAuth 证据（`STEAM_AUTH_COMPLETED=true`、历史 `steam_auth_done` 状态/阶段或非空历史邀请码）迁移为 enabled。单纯 `game_installed/ready_to_start/starting/running/stopped`、SteamCMD 完成态或 SteamCMD cache 均不是邀请码授权证据，必须迁移为显式 false，避免升级后无意物化 Auth。
- `STEAM_INVITE_RUNTIME_SCOPE_VERSION=1` 是独立的一次性运行栈收敛标记。fresh 实例直接写入；旧 enabled 实例零 Docker 清理并保留 session 后写入；旧 disabled 实例只按精确 `<compose-project>_steam-session` 查找 holder，并在任何删除前完整 inspect 同卷全部 holder：仅权威 Compose project/service label 的 `steam-auth` 与带 Panel owner/project label 的一次性 Auth 容器可删除，发现未知或伪装 holder 时 fail closed，保留全部 holder/volume 且不写 marker；全部通过后才删除 holder、volume 并写入，失败则在下一次 `Prepare` 重试。该迁移不 inspect/pull Auth 镜像、不调用 Auth health/Compose service start-stop，也不触碰 server、SteamCMD cache、共享凭据或 game-data。
- `Prepare` 对旧 Compose 幂等删除且只删除 `server -> steam-auth` 依赖，保留 Auth service 定义和其它用户依赖；block mapping/list 均保留原换行与权限，无法安全理解且包含 Auth 的 inline `depends_on` 会 fail closed、原文件不写。marker 写入后所有 disabled 日常生命周期仍保持零 Auth 查询/轮询。
- disabled runtime apply/dry-run/recovery 现在使用 server-only Compose config inspect、image validate、`ps`、stop/up/health；可选 Auth 镜像只以内部不可拉取占位值满足 Compose 变量解析，绝不进入选中服务、拉取、inspect、health、snapshot、session volume 或恢复计划。schema 4 事务以持久意图精确分流；schema 1～3 的历史中断事务仍保持保守 Auth 恢复语义，防止升级中途丢失旧快照。
- `runtime_update` 回归覆盖 disabled 的运行态/停止态成功、server health 失败回滚、required 自动 dry-run→apply、schema 4 中断恢复与 schema 3 保守恢复；真实 Docker server-only fixture 使用无效可选 Auth 配置仍能完成 config/validate/up/ps/inspect/stop，并断言 Auth 容器和 session volume 为零。
- Linux SteamAuth TTY 新增真实 Docker integration：验证 PTY、登录完成后的预期取消、精确命名容器清理、密码不进入宿主 argv/Panel 日志、只挂 session 且不挂 game-data。运行中一次性容器的 `Config.Env` 仍是敏感 inspect 面，诊断只能做脱敏投影，并须在终态精确删除容器。候选升级 fixture 还覆盖“旧实例仅有 SteamCMD 证据保持 disabled”与“旧实例有强 Auth 证据保持 enabled/session sentinel 不变”两轮真实 Web 升级。
- 以上能力已进入 `v0.6.0`。正式候选 `33073661356` 已完成 `v0.5.13 → v0.6.0` 与最低支持边界 `v0.3.2 → v0.6.0`；annotated tag、三仓唯一 digest、`latest`、正式 smoke 与 GitHub Release 证据见本文件顶部及 `docs/09-image-build.md`。

# STEAM-INVITE-STARTUP-WAIT-1：启动期邀请码缺失不再误报 Auth 异常（2026-08-26，released in v0.6.0）

- `lifecycleRunner.fetchInviteCode` 不再直接 `cat /tmp/invite-code.txt`：固定的无用户输入 shell 条件读取把“文件尚未生成”规范化为 `"", nil`，因此运行态正常等待由 `/invite-code` 返回 `status=generating`。文件已存在但真实读取失败、容器执行失败等错误仍向上传递。
- `GET /api/instances/:id/invite-code` 在实例仍为 `starting` 时把剩余执行错误视为启动过渡并返回 `generating`；后续 release preflight 又把同一语义扩展为上述专用 marker 驱动的 10 分钟 running 暖机窗口，只有窗口到期或 marker 不可信时才返回 `auth_unavailable`。不得用启动期自然为 false 的 `steamAuthReady` 推断 session 失效，也不得改写持久 Auth 完成态。
- 后台邀请码轮询与 HTTP 语义现保持一致：空码/缺文件继续重试，只有拿到非空码才写 driver payload；等待或失败都不影响 server、局域网/IP 直连和既有 Auth session。影响 `stardew_junimo/lifecycle.go`、Web lifecycle handler、driver/HTTP/真实 Docker integration 回归。

# CABIN-VANILLA-DEFAULT-1：小屋缺省统一为原版（2026-08-26，released in v0.6.0）

- `NewGameConfig.cabinMode` 继续接受兼容 wire 值 `recommended|vanilla`，但空值现统一归一为 `vanilla`；`newGameServerSettingsJSON` 以 `Server.CabinStrategy="None"` 为 fail-safe，只有用户显式提交 `recommended` 时才写 `CabinStack`。旧客户端和已持久化 job payload 不需要改字段名。
- `ReadServerRuntimeSettings` 在文件或字段缺失时也返回 `CabinStrategy=None`，Web GET 因而与新档原版默认一致；显式 `CabinStack`、`FarmhouseStack`、`None` 仍原样读取/保存。启动时的 `EnsureServerSettingsDefaults` 不物化缺失小屋字段，不把这次默认调整扩展成旧实例静默迁移。
- 后端日志 API、存储和 SSE 仍按 `sequence` 升序；安装页“最新在上”只是前端派生展示，`lastSeq` 与阶段推导契约不变。影响 `registry/types.go`、`stardew_junimo/saves.go`、Junimo/Web 回归测试；以下旧章节中“默认 recommended/CabinStack”已由本节取代。

# STEAM-INVITE-OPTIN-1：SteamCMD 默认安装与 Steam 邀请码按需启用（2026-08-26，released in v0.6.0）

- 新实例在实例 `.env` 中显式持久化 `STEAM_INVITE_ENABLED=false` 与独立授权状态；`steamInviteEnabled` 是用户意图，不能由邀请码是否为空或 `steamAuthLoggedIn` 临时推断。旧实例缺少新键时由 `Prepare` 一次迁移：只有历史 SteamAuth 完成态、`steam_auth_done` 状态/阶段或已有非空邀请码等强 Auth 证据保留 enabled；普通安装/运行状态和 SteamCMD 证据迁移为 false，显式 false 永远优先。
- 普通安装/修复只检查 JunimoServer 镜像并以 SteamCMD 一次性工具完成 app `413150`、Steamworks SDK app `1007`、SMAPI、Junimo/Control 准备和最终文件校验。disabled 链不得 pull/create/run/probe/轮询 `steam-auth`，其缺失不影响安装、启动、存档、Mod、备份或 LAN/IP 直连。SteamCMD 候选/latest 与授权缓存策略保留，但工具不进入 runtime stack manifest、digest 供应链、日常升级检查或版本 pin。
- SteamCMD 与 SteamAuth 共用实例 `.env` 中的 `STEAM_USERNAME` / `STEAM_PASSWORD`，前端统一称为“Steam 账号密码”；这份数据只是两条链在既有授权失效后的共享回退凭据，SteamCMD machine authorization cache、SteamAuth session 与各自完成状态仍独立持久化。管理员只能在基础安装态且 installation diagnostic 的 `requiredFiles=ok`、服务器已停服时使用 `PUT /api/instances/:id/steam-credentials`，并且只提交 `steamUsername/steamPassword`；未 Prepare/未安装请求以 `installation_required` 拒绝且不创建/修改 `.env`。接口不创建 job、不运行 SteamCMD/SMAPI/Auth，也不要求 VNC 或镜像字段，只原子更新上述两个 `.env` 键并返回不含凭据的权威意图/授权状态。修改账号密码不得清除或重建已经成功的 SteamCMD cache，也不得使已经成功的 SteamAuth session 失效；运行时始终先复用现有 cache/session，只有实际登录结果明确判定其失效后，才使用当前保存的凭据重新登录。缓存失效后切换完整登录或 exit 139 重试时会重置全部单次尝试状态；SteamCMD 客户端自身的 `Downloading update` / `Update state` 不是授权成功证据，只有明确 `logged in ok` 或 app `413150/1007` 成功标记才可重新写 `STEAMCMD_AUTH_COMPLETED=true`。邀请码失败/未完成后的重试只可清理 SteamAuth session，不得删除 SteamCMD cache、四个 SteamCMD 授权卷或 game-data。
- `POST /api/instances/:id/steam-auth/login` 保持管理员专用并创建独立 `stardew_steam_auth` job；enabled 只在 exclusive job 已持久化之后、runner 启动之前写入，任务同步创建失败不会留下没有 job 的 pending 意图。随后才按需检查/拉取 SteamAuth 镜像并启动带 Panel owner/project label 的一次性授权容器，Linux CLI 与 Windows Engine API 路径使用同一证明标签。账号密码模式运行无游戏下载的 `serve`，直接消费共享 `.env` 中的 `STEAM_USERNAME/STEAM_PASSWORD`，确认登录终态并保存 session 后由 Panel 取消、等待容器停止；二维码模式运行会自行退出的 `login` 并只预送 `2\n`，保存 session 后同样结束，绝不再进入会继续 `DownloadAll` 的 `setup`。真实登录成功一经观察就通过一次原子写入同时保存 completed、enabled 与 `ready|cleanup_pending`；one-shot 精确收尾成功为 `ready`，收尾失败为 `cleanup_pending`，后者可让本次 job 报错但不得撤销成功 session，账号不一致拒绝仍是例外。Auth-only 只挂载 session volume，`GAME_DIR` 使用容器内 scratch，绝不挂载 game-data，且不要求无关的 VNC 密码。
- 现场旧实现停在 `Choice [1]: 1` 不是 Steam 网络仍在下载，而是凭据模式误入 upstream `setup` 后继续等待交互式 username/password；Panel 只预送菜单 `1`，且无换行的用户名/密码提示不在 Guard prompt 白名单中，所以日志和任务同时悬停。凭据模式改用 `serve` 后消除了这个隐藏输入点，同时仍保留 QR/Guard 交互。
- Auth-only 的每一次 phase 更新和 Guard 主动状态推进都保留任务开始前的基础 `instance.State`；授权进行中、镜像拉取失败、登录失败、取消或成功都不会把 `game_installed/ready_to_start/stopped` 改成基础安装失败，完成时再恢复完整 state/message/phase/payload 快照。Guard 输入先校验 job 所有者、类型、运行态与 active-job lease，迟到输入不能改写新任务终态。Panel 中断时 jobs recovery 使用无凭据快照兜底；启动恢复只对 enabled 且 `failed/authorizing`、服务器未运行的实例精确清理 Auth holder/session，disabled 零探针且绝不扰动运行中的 server。已经 `ready` 且登录完成的 SteamAuth session 受幂等保护，重复启用/登录不得启动 job 或清 session；只有 `failed/pending` 的重试才以 Auth-only `ForceReauth` 清理 `STEAM_REFRESH_TOKEN`、`STEAM_AUTH_COMPLETED` 和 `_steam-session` holder/volume，但 holder 删除统一先执行上述全量安全分类，发现任何未知 holder 时保留 session payload、全部 holder/volume 与既有完成态并报错，不做部分清理；SteamCMD cache、四个 SteamCMD 授权卷与 game-data 始终保留。
- Compose 仍声明可选 `steam-auth`，但 `server` 不再有默认 `depends_on`。生命周期、存档维护、运行栈更新、SMAPI 更新和诊断按意图选择 `server` 或 `steam-auth + server`；支持包在 disabled 时只调用 server-only `compose ps`，接口不可用时写入该条目错误而不回退解析完整 Compose。disabled 的所有 Auth probe、自动刷新和后台邀请码轮询为零；enabled 保留既有严格 `/health`、session volume 保护和邀请码获取。
- `GET /api/instances/:id/state` 返回权威 `steamInviteEnabled`、`steamInviteAuthState`、兼容的 `steamAuthLoggedIn/Ready`；disabled 不执行 Auth readiness。`GET .../invite-code` 在 disabled 时无副作用返回 `steamInviteEnabled=false,status=disabled,inviteCode=""`；enabled 使用 waiting/failed/server_stopped/generating/ready/auth_unavailable 稳定状态。
- 本规则取代文档中的 `STEAM-AUTH-FLAG-1`、`INVITE-CODE-DECOUPLE-AUTHSTATUS-1`、`INSTALL-ROUTING-SPLIT-1` 与 `STEAMCMD-FALLBACK-1` 作为现行安装/邀请码契约；以下章节保留为历史发布记录。角色可见性与角色迁移工具不在本次范围。
- 主要影响 `config/env.go`、`installer.go`、`driver.go`、`lifecycle.go`、TTY runner、runtime/SMAPI/save-import 工作流、Web state/install/lifecycle/support handlers、jobs recovery 及对应测试。最新本机验证已通过 Auth/Guard/恢复/清理的 Go 定向测试、`go vet ./...`、`go build ./...`、前端安装/小屋/响应式回归与 production build；Windows 全包只剩既有 NTFS mode 差异，另一个无关并发夹具精确复跑通过。用户要求的热修改阶段没有新建容器，真实 Steam 账号、Guard/QR 与最终邀请码留在本机预览由用户验证；正式发布门禁、tag、latest 与 Release 不在本地任务中执行。
- 后续用户在既有热预览里自行完成一条真实 Steam Guard 授权；代理没有输入、读取或记录凭据。任务日志确认 session 保存后一次性 Auth 容器安全停止，投影检查确认 `STEAM_INVITE_ENABLED`、SteamAuth 完成态、SteamCMD cache、共享凭据存在性和 `_steam-session` volume 均保留，基础实例仍为已安装且 server/Auth 运行容器为 0。最新 native `dev` 后端 build date=`2026-08-26T14:15:48Z`，`/health=ok`；最终邀请码仍需用户启动服务器后验证。

# CONTROL-ONLY-AUTH-ADVISORY-1：未变化认证服务不再阻塞 Control-only 升级（2026-08-23，released in v0.5.13）
...
- 影响 `internal/docker/runtime_apply.go`、`runtime_update_apply_runner.go`、`driver.go`、apply 单元/真实 Docker integration 夹具，以及 Web full-stack 阶段映射；API 结构、SQLite、Compose、运行栈清单、Steam 登录后台策略与凭据均未改变。公开 `fullStack.phase` 在既有字符串字段中新增 `verifying_auth`，`verifying_runtime` 只再表示 Junimo/SMAPI/Control 验收。真实 Docker auth fixture、Linux Go 1.25 整仓 test（Junimo 96.341s、Web 87.857s）、Windows vet/build 已通过；Windows 全包唯一失败仍是已记录的 NTFS `0666`/Linux `0640` mode 差异。
- 首轮候选/Compatibility 因后台 `sleep` watchdog 成功路径反杀真实探针而失败，未生成可提升制品；改为 Bash HTTP 读取自身的一秒上限后，`be25fb3a4d0dfda4a9240a70e9fdb1d3a01a64cd` 的 Compatibility `32648758687` 与候选 `32648758732` 全绿。自动 annotated tag `32649334502` 和正式提升 `32649344923` 成功，能力已随三仓同 digest 的 `v0.5.13/latest` 发布；唯一 digest 为 `sha256:b983d444d82f3303dbe65aa130a6da4160beaa3c98bcffd5f3704724395071a9`。
...
- `scripts/tests/test_release_candidate_upgrade.sh` 最初为 v0.4.18 加入升级后空集合专项；v0.6.0 的 scoped Stop 已改变当前运行契约。现行夹具使用实例真实 project=`stardew`，公开 Stop 后要求恰好一个 `exited|dead` server，再上传伪存档并要求返回 `202/jobId/operationId`；受控 maintenance 启动失败后必须恢复 stopped、项目零运行容器，且只允许 server 不存在或保留一个 exited/dead 副本及默认 network，Auth/其它 orphan 为零。随后测试才精确 `down --remove-orphans` 进入下一场景；不能用产品 Stop 后 container/network 归零代替 scoped non-destructive 语义。
...
- **SteamCMD 与 steam-auth 的授权材料严格独立**：二者共用 `.env` 中保存的 Steam 账号密码，但 SteamCMD 不复用 `steam-auth` 的 refresh token，而是使用自己的 `config.vdf` / ConnectCache / machine authorization 文件；`STEAMCMD_AUTH_COMPLETED` 也不影响 `STEAM_AUTH_COMPLETED`。
...
> 本节记录旧版安装路由历史。`v0.6.0` 已移除公开基础安装请求的 `forceReauth` 字段；修改共享账号密码改走上文独立 `PUT .../steam-credentials`，不得启动安装或清理任何授权缓存/session。
...
- **两个对称的持久“认证成功”标志**（均写在实例 `.env`，`forceReauth` 时清空）：
  ...
  - 二者共用同一份 Steam 账号密码，但**授权缓存、session 与完成状态各自独立、互不牵连**：`STEAM_AUTH_COMPLETED` 由 steam-auth 认证成功或非空邀请码置位、决定“是否还需要跑 steam-auth 登录步骤”；`STEAMCMD_AUTH_COMPLETED` 只由 SteamCMD 登录成功置位、决定“SteamCMD 用缓存还是完整登录”。SteamCMD 登录成功**不会**顺带把 `STEAM_AUTH_COMPLETED` 也置位。
...
- 2026-08-31 响应扩展为 `{ ip, checkedAt, source?, cached, gamePort?, protocol? }`。公网 IP 仍按服务器级结果缓存 `10min`；实例端口不进入该缓存，而是在每次响应时通过可选 `registry.DirectConnectConfigProvider` 从对应 driver 读取。Stardew driver 使用实例 `.env` 的 `GAME_PORT`，缺键遵循 Compose 的同一默认 `24642`，只接受 `1..65535`，协议为 `udp`；非法值只省略连接字段并记录服务端 warning，不把猜测端口交给前端。
- 请求 `?refresh=1` 或 `?refresh=true` 只强制重新检测公网 IP。公网检测失败仍返回 `502 public_ip_failed`，不会暴露外部服务原始错误；不支持 direct-connect capability 的其它 driver 保持原响应兼容。
- 影响文件：`backend/internal/games/registry/types.go`、`backend/internal/games/stardew_junimo/{driver.go,config/env.go}`、`backend/internal/web/public_ip.go` 及对应测试。未改 Docker/Compose、邀请码接口、实例状态模型或实例创建契约。
- 验证：`cd backend; go test ./internal/games/stardew_junimo -run "TestDriverDirectConnectConfig|TestDriverIdentity" -count=1`；`go test ./internal/web -run "TestPublicIP" -count=1`；交付前另跑两包完整回归。
...
- owner 首次抢占使用完整 staging 目录：先以 `0600` 写入 `owner.json`，再 fsync 文件/目录，最后以 Linux `RENAME_NOREPLACE` 或 Windows 不覆盖 `MoveFile` 原子发布；并发抢占只有一个 winner。Linux 在 Docker Desktop 9p/DrvFS 上仅当 `renameat2` 明确返回 `EINVAL`、`ENOSYS` 或 `EOPNOTSUPP` 时，才退回到“独占创建目标目录，再移入已校验并已同步的 `owner.json`”；目标已存在时绝不替换，发布中断只清理本次创建且仍为空的不完整目录。历史空 owner 目录只在早于当前进程、内容确实为空，且不存在 rotation、非终态事务、marker/catalog 或 Control 创建进展时才可隔离恢复；任何未知内容都 fail closed。
