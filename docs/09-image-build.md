# v0.6.0 SteamCMD 默认安装与邀请码 opt-in（2026-08-27，发布预演中）
...
- 专用暖机 marker 完成后，任务专属 Linux Go 1.25.11/DinD owner=`anxi-v060-final-gates-20260827-1209` 已再次通过整仓 `go test ./...`（Junimo `87.035s`、Web `56.122s`）、`go vet ./...`、`go build -o /tmp/anxi-v060-panel ./cmd/panel`，owner container/network/volume 独立精确复核为 `0/0/0`。该结果只是代码门禁；下面两条真实 Web 升级、不可变候选、proof、annotated tag、promotion 与正式 smoke 仍待完成。
...
- 当前正式基线是 `v0.5.13@be25fb3a4d0dfda4a9240a70e9fdb1d3a01a64cd`，正式 digest=`sha256:b983d444d82f3303dbe65aa130a6da4160beaa3c98bcffd5f3704724395071a9`。`v0.6.0` 必须从最终干净且同步的 `main` 构建新不可变候选；任何本机 `dev`、旧本地 tag 或中间镜像都不能被提升。
- 本次 final release commit message 必须包含精确 marker `[manual-release-candidate]`：push-origin 自动补丁候选及其无 proof 的下游 Tag job 会明确 skip，随后只允许从同一 `main` SHA 手动 dispatch `0.6.0`。共享 `scripts/validate-release-matrix.sh` 在候选、自动 Tag、正式提升三段都强制 `oldest < previous < version`，并以版本控制映射把 `v0.6.0` 精确锁定为 previous=`0.5.13`、oldest=`0.3.2`；三段都会重新读取当前 latest GitHub Release，首次候选/Tag/promotion 路径要求 proof previous 与其一致，promotion 仅保留下条所述同 version 幂等重跑例外。显式零 patch 版本还必须携带 oldest，因此替换/漏跑任一升级边界、篡改 proof 或拿自动 `0.5.14` 抢占本次发布都会 fail closed。
...
| 维度 | 必测场景 | 通过标准 | 当前状态 |
| --- | --- | --- | --- |
| fresh 默认安装 | 新实例无 Steam/Auth 资源，以 SteamCMD 完成 413150/1007、SMAPI、Junimo/Control 和校验 | 安装成功；Auth pull/create/run/probe/poll 为零；LAN/IP 可用；SteamCMD 不进入 runtime manifest/pin | 单元/既有本地安装证据已覆盖；候选 fresh 待执行 |
| 旧实例迁移 | 缺新键且分别只有 SteamCMD/普通安装证据、具有强 Auth 证据、已有 explicit false/true | 前者持久化 false；强 Auth 保持 enabled；explicit 不被覆盖；重复 Prepare 幂等 | env/driver 回归已覆盖；真实升级 fixture 待候选执行 |
| 按需 Auth | 管理员凭据/QR/Guard、成功后取消、cleanup error、ready 重复请求、failed 重试、未知/同名伪装 holder、修改共享凭据 | PTY 正常且 one-shot 带 Panel owner/project label；密码不进入宿主 argv 或 Panel 日志；运行中的一次性容器 `Config.Env` 仍属于敏感 inspect 面，只做脱敏投影；只挂 session，成功先持久化且 cleanup error 不销毁 session；未知 holder 零部分删除；不下载游戏、不清 SteamCMD cache/game-data | Linux 真实 TTY integration 与 classifier/恢复/cleanup 定向回归通过；候选升级后复验待执行 |
| disabled 生命周期/升级 | running/stopped apply、required dry-run→apply、server health 失败 rollback、schema 4 interrupted recovery、无效 Auth image、支持包 | 仅 server 的 config/validate/ps/stop/up/inspect/health；支持包全量 Compose ps=0；Auth image/container/health/snapshot/session/poll 为零；失败恢复旧 server 状态 | 单元、Web 支持包和真实 server-only Docker integration 已通过；候选 updater integration 待执行 |
| 历史恢复兼容 | schema 1～3 的旧 full-stack 事务在新 Panel 恢复 | 按旧事务保守处理 Auth，快照/session 不丢；不套用 schema 4 精确 false | 回归已覆盖；`v0.3.2` Web 直升后复验待执行 |
| save-import completed commit / rollback panic | active runner 下 Prepare、状态发布注入失败重试、server absent/exited/unknown/duplicate 与 Auth absent/exited、owner 缺失兼容与 malformed/mismatch/multiple legacy、enabled 二次发布、其它 phase、全部 rollback substages、swap/as-is/completed panic | active runner 零恢复；completed 不回滚且幂等发布 running；Auth 不阻断基础运行而 server 必须唯一 running；坏 owner/orphan/multiple 保守停服且不恢复，已确认 completed 后失败零 stop；二次发布不刷新 marker/不覆盖其它 phase；部分回滚不 resume；panic 原值不外泄 | Windows 定向测试与最终 Linux 全量已通过；`v0.5.13`/`v0.3.2` 同镜像升级待执行 |
| API/权限/前端 | disabled invite GET、普通用户、enabled waiting/failed/stopped/generating/ready/auth unavailable、桌面/移动 | disabled 200 稳定 DTO 且零副作用；普通用户无按钮；LAN 常驻/Steam 条件显示；starting/有效暖机 marker 内等待，marker 不可信或到期后进入真实异常；前端预算有界且 disabled 零 fetch/poll | Go 暖机边界、前端状态/响应式回归与 production build 通过；候选 bundle/Browser 待执行 |
| 小屋/日志/job URL | fresh 建档省略 cabin、显式堆叠、旧三值、安装日志和 202/409 新 job | 默认 `vanilla/None`；`recommended→CabinStack`；旧值不迁移；顶部最新且共享 SSE 升序；URL 不被旧 job 覆盖 | 定向 Go/前端回归通过；候选 production build 待执行 |
| 上一正式版升级 | `v0.5.13 → v0.6.0` 同一候选先 unhealthy 后 healthy；另注入同卷未知 holder | Web check/dry-run/confirm/apply/断线重连；`failed_rolled_back/health_check_failed` 后旧版恢复；running 的旧 disabled 官方 Auth/session 由候选自身收敛且 server ID/StartedAt 不变；未知 holder 下 Web 升级成功而该实例 runtime-scope 收敛 fail closed，holder/volume/marker 全部保持，精确移除任务 holder 并重启后才收敛；healthy 后重跑本表专项与 Panel restart | **真实夹具已实现；正式候选必跑，待执行** |
| 最老支持边界 | `v0.3.2 → v0.6.0` 历史 apply 兼容 | 完整 Web 直升；SQLite/初始化/用户/实例/存档/Mod/备份/审计/非目标资源保持；两类旧 Auth 意图迁移正确 | **因长期意图迁移受影响而必跑，待执行** |
| 候选与正式提升 | final SHA 的 code gates、fresh/restart、proof、tag/promotion/smoke/Release | annotated tag 精确指向 proof commit；三仓版本/`latest` 六引用同 digest；OCI identity、health/version、Release 资产一致；promotion 不 rebuild | **待候选完成** |
| 隔离与清理 | 单元/真实 Docker/候选 DinD/两条升级/正式 smoke | 任务专属 owner/container/network/bind/volume；精确清零；不使用生产数据/长期凭据，不 prune | 局部真实 E2E 已清理；候选与正式 runner 结果待回填 |

- 2026-08-27 classifier 定向收口轮通过安全 classifier/one-shot label、disabled migration、Auth retry/recovery、成功后 cleanup error 保留 session 与 disabled 支持包 server-only 的定向三包测试，以及 `go test ./internal/docker -count=1`、`go test ./internal/web -count=1`；该定向轮未启动长 Docker。随后完成的最终 Linux 全量门禁与资源清零结果见本节顶部补充。Windows Junimo 整包仍只命中既有 POSIX mode 夹具差异；当前仍没有构建、推送或发布候选。具体测试名与文件清单记录在最新 backend handoff，正式证据仍以本表待执行的隔离 Linux/DinD 候选矩阵为准。
...
- 专用暖机 marker 补丁完成后，Windows 已再次通过 `go test ./internal/web -count=1`（`36.324s`）与 Junimo 包除既有 NTFS/POSIX mode 夹具外的全部测试（`100.944s`），新增 start/restart/恢复写入、普通 payload 保留以及 missing/bad/future/expired marker 的 HTTP 边界均通过；前端 19 项已声明状态/响应式脚本全部顺序通过，shared projection 收口后又重跑 install-state、responsive 与 `npm run build`（Vite 8.0.16，149 modules），deferred 用例覆盖 state/invite 两种逆序、disabled 零后续 GET、ready 不回退与运行代预算重置。随后上述 Linux 整仓复跑也通过；真实双升级仍以后续同一 clean commit 门禁为准。

## 候选、Tag、正式提升与 Release 证据（待完成）

- 本地不发布双升级预演仍待执行：最终 clean commit 必须用同一 version/commit/build date/image 分别完成 `v0.5.13 → v0.6.0` unhealthy/healthy 与 `v0.3.2 → v0.6.0`，两轮 metadata image ID 必须一致；本地通过不能替代远端正式候选 proof。
- 正式候选 workflow：待带精确 `[manual-release-candidate]` marker 的最终 `main` commit 推送后，以 version=`0.6.0`、previous=`0.5.13`、oldest=`0.3.2` 手动 dispatch；marker 只抑制该 push 自动推导出的 `0.5.14` 候选，不影响 workflow_dispatch。零 patch 的显式候选缺 oldest 会在构建前失败；oldest gate 使用相同 version/commit/build date/tag 再跑真实 Web 升级并强制比较 image ID，proof 记录 `previousVersion` 与 `oldestTestedVersion`，Tag/promotion 会再次校验。当前没有可提升的 workflow ID、candidate ref、proof artifact 或 digest。
- 远端正式候选内的真实 Web 升级：`v0.5.13 → v0.6.0` unhealthy/healthy 与 `v0.3.2 → v0.6.0` 均待实际执行；任何一条失败都停止 tag，不得降级为发布后观察。
- annotated tag workflow、tag object/peeled commit、正式 promotion workflow、三仓六引用 digest、OCI version/revision/created、正式 GHCR smoke、GitHub Release 和资产：全部待候选成功后由自动链生成并核对，禁止提前填写占位数字或复用 v0.5.13 证据。
- 发布完成后另以 evidence-only 提交回填实际 workflow ID、唯一 digest、选择/跳过矩阵、耗时、故障与精确资源清理结果；该提交不得移动 `v0.6.0` tag、重建候选或改变正式 digest。

# 启动期邀请码等待态修正（2026-08-26，本地未发布）
...
# 原版小屋默认与安装日志最新置顶（2026-08-26，本地未发布）
...
# Steam 邀请码授权卡死与安装状态隔离补修（2026-08-26，本地未发布）
...
# SteamCMD 默认安装与 Steam 邀请码按需启用（2026-08-26，本地未发布）
...
- 官网与长期文档回填提交 `616de0bd56999089530f98e729273b116507b994` 只触发 Docs Portal `32649797827` 和 Compatibility `32649797822`，两者均成功；Docs build 用时 22 秒、deploy 10 秒，Compatibility 约 2 分 28 秒。GitHub Pages API 确认 workflow 部署地址为 `https://anxiyizhi.github.io/stardew-server-anxi-panel/`；线上首页与 `/changelog.html` 均返回 HTTP 200，并分别命中 `v0.5.13`、Control-only 摘要、“正在验证认证服务健康（不等待 Steam 登录）”和“正在验证 SMAPI 实际加载版本”。该 docs-only push 没有触发 `Validate release candidate`，annotated tag 仍指向产品候选 commit，正式 digest 未改变。
