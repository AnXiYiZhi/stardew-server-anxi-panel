...
## 2026-09-19：深度减负联调收口（本地完成，未发布）

- 构建依赖分类、生成目录排除及内部孤立实现清理保持 API/driver/数据契约；已有直连接口、认证 UI 和素材改动保全。Linux tmpfs 全回归 1,924 pass/8 skip，vet/build/tidy 通过，磁盘临时目录的既有时限波动单列于 docs/07-later-optimizations.md。
- 最终 Docker 镜像 24 项真实 API 验证通过：health/version、未初始化状态、重复初始化拒绝、登录/注销、匿名与普通用户权限、HTML/静态资源，以及独立数据卷的完整用户列表和 session 跨容器重启。前端 52 组渲染/导航检查使用隔离夹具；本轮未执行真实 Steam 和正式 Web 升级/回滚门禁，不作对应通过声明。
- Docker/WSL 故障已恢复。stardew-2-server-1、anxi-install-test-20260905、anxi-panel 均 running/healthy，两个 Panel health/database 正常；原六容器和 23 卷保留，本任务临时 Docker 资源清零。证据与边界见 docs/project-load-audit-2026-09-18.md。

## 2026-09-18：全局减负联调记录（本地完成，未发布）

- 旧独立安装页面和前端无人调用的 API 包装已清理；`GameInstallRail` 继续向实例 `/install` 提交凭据并跟踪指定任务，旧安装路由继续按现有游戏库跳转契约运行。服务端既有端点、鉴权、响应结构、driver 能力及当前直连接口改动保持。
- 验证覆盖前端全部声明测试、46 组浏览器页面/视口与手机导航、完整静态资产引用；后端 Linux 回归及 vet/build，安装失败持久化超时用例独立 3 次通过。当前为隔离接口夹具与代码回归，正式部署验收仍由后续不可变候选执行。

## 2026-09-18：直连地址独立读取世界端口（本地完成，未发布）

- 新契约：登录态 `GET /api/instances/:id/direct-connect` 返回 driver-owned `{gamePort, protocol}`，与公网检测结果独立。未登录 401、不存在实例 404、非 GET 405、配置读取失败 500 `direct_connect_config_failed`；不支持 capability 返回 400 `direct_connect_unsupported`。
- 消费链：世界列表、桌面与移动总览读取该接口，并以当前面板 `hostname` 拼接游戏端口；默认 24642 省略，其它合法端口附加，IPv6 自定义端口加括号。停止状态可用，显示与复制一致。
- 验证：真实 Go HTTP handler 定向回归覆盖外部 IP 服务不可用而配置可读、多世界与配置刷新、权限/错误边界；Chrome 隔离 API 夹具验证 1280×800/390×844 切换、复制和同步，断言公网查询零请求。前后端构建和相关回归通过。
- 后续：候选中补真实 Docker 全新/升级后专项。本次没有更改旧 `/public-ip` 的出站查询用途或兼容响应。
