# 2026-09-10：人物删除恢复入口修复

- 桌面/手机继续共享原 `FarmhandDeleteFlow`。提交成功直接采用后端返回的持久意图；响应不确定时重新读取，管理员页复用现有 3 秒轮询发现操作，避免后台仍等待删除但页面无取消入口。
- hook 按已应用的请求序号拒绝旧响应及旧错误，避免较早的 GET 抹掉已接受的等待或倒计时；慢于轮询间隔的响应仍可依次推进，不要求所有请求在 3 秒内完成。
- 手机保护备份入口复用完整管理页的导航能力，进入“其他备份”并显示对应 `prefarmhanddelete` 备份；不另建一套手机备份界面。QA recovery 夹具包含同名保护备份。
- 23 项前端状态测试、生产构建通过；Browser 已实际验证手机进入同名回档确认框，以及 POST 响应丢失后等待/取消仍可见。本轮未发布前端或替换生产容器。

# FE-FARMHAND-DELETE-MAINTENANCE-2：等待与立即维护删除（2026-09-09，待审核）

- 桌面与手机玩家页共用 `FarmhandDeleteFlow`。默认选择“等待无人在线（推荐）”；“立即进入维护（不推荐）”才显示当天进度风险勾选和目标人物名二次输入，并明确 60 秒内每 10 秒通告、之后断开真人玩家、睡觉或日结算会取消。
- `useFarmhandDeleteIntent` 在管理员页面每 3 秒读取意图，包括初始无意图时。状态区支持取消等待、取消倒计时、重试最终保存和前往保护备份；非终态意图会禁用其它人物删除入口，页面刷新或 Panel 重启后仍可恢复显示。
- API 调用显式携带当前 `instanceId`、联机 ID、显示名、预期 saveId、mode 和风险确认，避免多世界操作落到旧默认实例。新增 `test:farmhand-delete` 并接入 compatibility/release gate；全部前端状态脚本、TypeScript 与 production build 已通过。Browser 在 1280×720、390×844 验证等待/立即弹窗、双重确认门禁和 recovery 卡，root/body 无横向溢出，干净标签 console warn/error 为 0。
