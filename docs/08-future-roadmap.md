# 2026-09-10：人物删除审查修复状态

- [x] 修复真实门禁、人数证据、过期命令、恢复 CAS、恢复启动、保护备份定位和切档取消；去掉重复 kick/人数函数/实例转换。
- [x] 修复手机保护备份跳转和不确定提交的状态恢复；完成本地回归、真实 Control 编译和隔离 Panel 迁移 017/登录/重启烟测。
- [x] 实机验证并修复手动保存合成 DayStarted 误取消、保存尾菜单误取消、非默认主机 API 映射下删除请求端口错误。
- [x] 最终 Linux 全量 test/vet/build 退出码 0；真实双客户端全链 541.02 秒 PASS，包括 06:10 拒绝重连、SIGKILL 重启仍锁定、指定备份恢复、睡觉取消和最终保存重试。证据已下载，专属测试资源清理完成，三个现有容器身份/启动/重启计数未变且健康。
- [x] 作者复审补齐 seal 回执丢失且释放失败的恢复入口、前端旧响应覆盖已接受意图；新 Go 回归先失败后通过，前端状态脚本与构建再次通过。原实机结果与最终补丁的验证范围分别记录在镜像构建文档。
- [ ] PR 合并后，正式发布前执行最终源码的不可变候选、真实 Web updater/回滚、tag 与镜像提升；本任务不替换生产服务。

# 2026-09-09：离线人物等待删除与维护闸门（待审核）

- [x] 两种入口：默认等待无人在线 24 小时；显式立即模式 60 秒倒计时并每 10 秒游戏内通告。
- [x] countdown 可原子取消；进入 active 后不可取消。目标上线、切换存档、等待超时、睡觉或日结算均在删除边界前取消。
- [x] 保持世界进程运行，Control 以 offline 关闭新连接并断开真人玩家，Panel 确认零连接；正常完成不重启服务器。
- [x] 删除前定向保存和整档保护备份；Junimo DELETE 后再次定向保存并验证磁盘。后半段不确定时保持联机关闭并提供最终保存重试/保护备份入口。
- [x] 迁移 017、后台调度、中断恢复、桌面/手机 UI、API/状态/C# 契约和前端门禁已实现并通过本地定向验证。
- [x] 2026-09-10 用户授权后已完成真实 Junimo Docker 双客户端删除/倒计时/断开/睡觉/恢复验收；正式候选升级与发布仍未执行，详见本页顶部及镜像构建记录。
