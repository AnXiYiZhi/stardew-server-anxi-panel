## 2026-09-15：面板按钮与竖屏操作统一（已修复，未发布）

- 公共按钮改用新生成的 `button_frame_warm_9slice.png` 九宫格边框：固定 5px 边角、独立语义底色，默认/行内尺寸为 32/28px，紧凑壳保持至少 44px 触控高度；补充键盘焦点与减少动态效果规则。普通、主操作、危险操作共享边框材质。
- 移动总览启停操作改为等宽双列，版本入口显式定义背景；控制页统一左对齐图标/文案并允许说明换行；玩家卡片保留 Mod/回家，将低频管理动作放入原生 details，原权限、禁用条件和确认弹窗保持。桌面设置的用户信息与操作分行，Mod 危险按钮复用公共底色。
- 影响 `stardew-theme.css`、`mobile/Mobile{Home,Control,Players}Page`、`pages/{Settings,Mods}Page.css`、响应式回归及新增 PNG；没有后端/API 变化。
- 验证：package.json 全部 test:* 与 production build 通过；隔离 QA 夹具 390×844 总览/控制/玩家、320×740 玩家和 1440×1000 设置无页面横向溢出。390px 总览启停均为 161×44px，边框固定；语言弹窗开关和玩家管理 Enter 展开通过。使用合成数据，未触发真实游戏操作。
- 下一步：正式候选覆盖共用按钮的核心流程；新增按钮复用九宫格与语义变量，不再整图拉伸。当前为源码修改，未部署正式镜像。
