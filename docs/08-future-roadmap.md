...
- 续查已完成的本地项：4 份退出使用的旧 Go 可执行缓存无损归档，净减少 49,020,744 B，保留可恢复原字节；开发 build 通过，其新增编译缓存和审计开销单列。用户追加授权后 Git 保留全部 78,947 对象重新打包，减少 854,438,129 B，refs/reflog/index 不变且前后完整 fsck 通过。candidate.tar、恢复夹具和共享资源继续保留。详细边界见审计报告续查节。
- 已执行并验证：9 个内部孤立 Go 实现清理、前端构建依赖分类与 npm ci、Docker 生成目录排除、两份可重建旧二进制删除（30,186,725 B），以及官网一处失效官方链接修复。初始 114 个差异文件与 5 个业务新增文件单独保全。
- 实测三组 Docker COPY 输入 263,512,784 → 22,079,473 B；最终镜像 59,417,588 → 59,417,134 B，镜像仅减 454 B。前端 public/dist 保持；审计归档新增占用单列，不把逻辑缓存统计当宿主磁盘净收益。
- 验证：27 项前端脚本/build、52 组浏览器检查、website docs:build；Linux tmpfs 1,924 pass/8 skip 和 vet/build/tidy；真实 Docker 24 项初始化/权限/重启持久化冒烟。原三个运行容器恢复健康，测试容器/卷/镜像及八条上下文缓存清理完成。
- 保留项：业务数据、授权、恢复夹具、无法证明可重建的镜像归档、活跃依赖/共享缓存、设计与授权历史。磁盘临时目录的既有安装错误回归时限问题转入 docs/07-later-optimizations.md；Control 未改，未重编译。详情见 docs/project-load-audit-2026-09-18.md。

## 2026-09-18：服务器与玩家页响应式布局修复（本地完成，未发布）

- 已完成：清理影响服务器/玩家网格的重复视口断点，解决服务器左侧挤压与右侧空白、玩家活动被拉成全宽；活动卡片按内容确定高度，继续使用统一浅奶油底色。
- 已验证：响应式关联回归与 production build；Browser 四种桌面尺寸、跨页样式加载、地址完整性、玩家活动分页。有/无待认证区均覆盖，补充隔离直连端口夹具。
- 状态：本地完成，未发布；详细文件、验证和容器断点维护约定见前端文档及最新接手记录。

## 2026-09-18：统一浅奶油纸色（本地完成，未发布）

- 已完成：按用户确认的总览纸色建立共享 `--sd-card-bg`，覆盖桌面管理页、移动卡片、登录/初始化、暖昼游戏与世界列表及共用确认框；连接/表单区域同步，各页面沿用原有布局。
- 已验证：响应式关联回归、游戏库状态测试和 production build；应用内 Browser 核对桌面七页、390px 手机五页、登录与世界列表，实际底色一致且无页面横向溢出。
- 状态：本地完成，当前未发布；后续纸色调整统一通过主题变量，详细文件与验收见前端文档及最新接手文档。

## 2026-09-18：总览农场手册样式（本地完成，待视觉确认）

- 已完成：总览浅奶油纸色、10px 圆角、字体层级、连接内容整行居中、事件名称与时间列、模组 2×2 圆角四格、透明祝尼魔空态；近期事件统一通过“查看全部”进入任务日志。窄窗口下顶部信息保持一行、服务器控制保持左右排列，低高度窗口收紧纵向间距。顶部横幅素材与宽窗口信息条、整体布局保持，样式仅在总览生效。
- 已验证：响应式关联链、命令结果、生命周期、production build；初版 Chrome 10 组状态/视口场景和设置、确认取消、导航交互，无相关控制台错误、缺失素材或横向溢出。使用隔离 API 夹具，未执行真实服务器写操作。
- 状态：本地实现完成；用户已要求统一底色，当前全局纸色工作见上方章节。总览布局细节保持本页独立维护，当前未发布。实现及验收细节见前端长期文档和最新前端接手文档。
- 细化验收：响应式回归与构建通过；应用内 Browser 验证四格与事件入口，以及 841×700、900×900、1035×700、1280×720、1473×1033 五种尺寸，覆盖停服/运行；主内容无需纵向滚动、底部按钮完整可见、无横向溢出，连接内容按整行居中。视觉验收以本线程实时总览预览为准。

## 2026-09-18：全局素材与冗余代码减负（本地完成，未发布）

- 已完成：全仓素材/模块/样式/Go 引用扫描；删除 48 个无效 PNG，清理旧独立安装页、未用 API/类型/辅助函数、25 个无调用者 Go 函数，以及失效 CSS 类与覆盖声明。现有工作区功能改动保留。
- 已验证：前端 27 项声明测试、production build、46 组浏览器视口与页面对照，资产引用完整；后端 Linux 全包回归（首次一个超时父测试隔离重跑 3 次通过）及 vet/build。前端产物约 28.45 → 15.44 MiB，减小约 46%。
- 状态：本地 main 完成，未发布；详细清理边界、验证证据和接手注意事项见前后端长期文档。本版候选仍按既有自动路径门禁执行。

## 2026-09-18：登录卡片视觉优化（本地完成，未发布）

- 注册页全屏补验完成：1920×1080 的 14 个核心元素位置及完整计算样式与原版一致；侧栏支持独立全屏效果预览。
- 登录按钮居中文字与注册页预览已完成；1100/390/320px 注册布局浏览器检查、production build 通过。
- 已完成：紧凑登录卡片的标题层次、奶油色双边框、输入框/按钮、内嵌密码显隐与右上角版本号；窗口宽度最大 360px，手机最大 320px。登录与初始化共用 AuthCard，全屏保持原场景和木框布局，按原断点切换并保留表单状态。
- 已验证：会话和响应式回归链、production build，登录/初始化接口夹具；最终 1920×1080 全屏截图与原版一致，1486×990/390×844/320×640 卡片尺寸、版本位置和往返缩放状态检查通过，无横向溢出或脚本错误。
- 状态：本地 main，未发布；后续正式候选抽验真实登录及移动浏览器输入。

## 2026-09-18：直连地址独立读取世界端口（本地完成，未发布）

- 已完成：直连展示从世界专用 `/direct-connect` 读取 driver 端口，配合面板访问 hostname，不再被外部公网 IP 检测失败阻断；桌面、手机和世界列表统一，停服支持读取。
- 已验证：后端连接/driver 定向回归、vet/build，前端游戏库/响应式回归及 production build；Chrome 桌面/手机接口夹具验证默认/自定义端口、世界切换、复制、同步及零公网检测请求。
- 待发布：仍为本地 main 改动；下一不可变候选补真实 Docker 新装与升级后的直连专项。

## v0.7.1 全量工作区发布（2026-09-17，已发布）

- [x] 2026-09-18 官网页底版本复用 frontmatter.release，changelog 与 GitHub Release 补齐未进入世界玩家占位修复说明；桌面/手机构建与点击验收通过，上线证据见 docs/11-docs-portal.md。
- [x] 修复候选夹具依赖与接口版本契约，完整候选、升级/回滚、自动 tag、三仓提升及发布后独立核验全部完成；门禁保持完整。
- [x] 官网 v0.7.1 首页与更新日志已上线；Pages 35239660559 成功，线上正文、版本顺序及 HTTP 200 已复核。
...
- [x] 普通玩家快照刷新与持久化安全检查分离，保留跨世界/存档和实例代际保护。
- [x] 最大人数独立短缓存，3 次新鲜快照的命令数 12 → 6。
- [x] 玩家心跳 30 秒持久化节流、业务变化即时写入、同轮 upsert 批量事务与回滚。
- [x] 存储按 driver 归属卷只读扫描，共享去重、缺失不创建、取消回收；真实 Docker 验证通过。
- [x] 构建/路径/编码级静态内容缓存，304 热路径消除全文读和哈希。
- [x] 自检补齐“所有调用者取消后共享读取失败”的缓存重试边界。
- [x] 任务容器、独立 Go 缓存卷、扫描器及 E2E 夹具回收，临时测试脚本删除。
- [x] Linux 全量后端回归已执行，性能专项 10 轮、前端相关回归与 vet/build 通过；千行安装日志夹具的 15 秒超时在隔离临时目录连续 3 轮复验通过，保留常规磁盘整包未一次全绿的限制，正式候选复核。
- 本机 1.49 MiB 合成资源 304 基准：约 1.15 ms / 1.50 MiB 分配 → 1.16 μs / 1.45 KiB；生产性能需按真实数据量采样，本轮未发布。

## PERF-READ-PATHS-1：八项性能优化（2026-09-17，本地完成，未发布）

- [x] 运行组件展示共享缓存，消除重复 Docker 探测与展示路径的 du 扫描。
- [x] 玩家主存档单次解析、文件元数据缓存、按实例并发合并与重复写入抑制。
- [x] 慢玩家读取移出 driver 全局锁，落库保留所有权和状态复核。
- [x] 14 张界面/登录素材压缩缩放，初始化轻量背景。
- [x] 静态 gzip、ETag/304、哈希/公共资源差异化缓存。
- [x] 桌面/手机路由预加载，诊断/总览跨页面复用展示数据。
- [x] 存储单次后台刷新，CPU/内存立即返回最近样本。
- [x] 普通 GET 超时、在途合并、可见性轮询与卸载取消。
- 本地证据：27 个前端回归脚本、生产构建、Linux vet/build、性能专项 ×10、两条真实 Docker E2E、桌面/手机隔离浏览器均通过。Junimo 整包（222.9 秒）与 Web 整包（43.4 秒）终验通过；任务容器、两个 Go 缓存卷及 18157 预览监听均已清理。
- 发布状态：仅本地实现；尚未创建候选、tag 或部署。正式门禁按 docs/09-image-build.md；真实生产性能数据待部署后观测。


## INSTALL-ERROR-EXPLANATIONS-1：安装失败原因与处理建议（2026-09-17，本地完成，未发布）

- [x] 安装环境错误审计：40 类明确中文原因与处理建议，所有非零退出码有中文兜底；code 5 结合当前日志判断，未知时不猜原因。
- [x] 后端保留当前失败尝试的原因，任务结果与前端错误摘要统一；列表/详情等时快照不丢诊断，小屏错误可见且原始日志可展开。
- [x] 共享样例、持久化/API 专项、安装/响应式状态回归、5 类桌面/手机浏览器交互和 Docker 前端构建完成。
- [x] Linux installerrors/Junimo 整包 1378 个用例/子用例通过、2 个既有条件跳过、零失败；相关 vet/Panel build 通过。临时容器、前端测试镜像和 4671 监听已清零。正式发布前仍需按镜像文档的候选流程完成本版专项和 Web 升级/回滚验收。
...
## 2026-09-17：世界 VNC 密码继承（本地完成，未发布）

- 新建世界继承首世界 VNC 密码；源密码缺失时返回 `vnc_password_required` 并回收创建资源。已有世界自定义密码由镜像兼容收敛保留，env 特殊字符读写已补回归。
- driver/Web 与配置回归、vet/build 通过；本机第二世界已保存并重启生效，运行态密码与首世界一致，VNC 使用 `VncAuth`，容器 healthy。后端创建规则待加载新构建，未发布。

## 2026-09-17：总览待批准玩家入口（本地完成，未发布）

- 批准按钮采用统一绿色木纹像素皮肤、浅色文字，最小宽 88px、高 32px；production build 与桌面/窄桌面浏览器复验通过。

- 总览在线玩家按 host、待批准玩家、其他在线玩家排序；待批准行右侧可直接进入批准确认，所有待批准玩家优先显示。沿用现有认证接口、管理员权限和命令结果校验，成功后自动刷新。
- 命令结果/响应式回归、production build 与 Chrome 隔离夹具的桌面/窄桌面、取消/成功/失败、权限及能力边界验收通过。后续正式候选补充真实 Junimo 待认证批准闭环；当前保留本地。

## 2026-09-17：玩家活动随认证状态排布（本地完成，未发布）

- 玩家页活动卡片在未开启密码认证时靠左，开启时与左侧待认证区域以等宽等高双列并排显示在右侧；窄容器保持单列，各卡按内容自适应。
- 响应式回归链、production build、真实双世界桌面布局、活动分页及 900px 窄屏验收通过；1473×977 等高补验确认两卡上下边缘一致。后续候选抽验认证状态变化与分页布局。

## 2026-09-17：服务器控制与消息布局（本地完成，未发布）

- 生命周期控制与全服消息在顶部同排，快捷操作在下方自适应横排；运行时显示停止/重启，可启动状态显示启动，过渡状态显示加载反馈。
- 修改 `ServerControlPage.tsx/.css`，沿用既有接口与权限。生命周期状态、响应式回归链和 production build 通过；Chrome 1473px 真实运行态、900px 停止态夹具和 390px 移动入口验收通过，确认框/语言弹窗开关及消息输入正常。保留本地未发布，后续候选抽验过渡状态与长消息。

## 2026-09-17：默认端口省略与直连布局（本地完成，未发布）

- 世界卡、桌面与移动直连统一在 24642 时省略端口，其它端口显示 `主机:端口`，复制内容一致；详情页已接入当前实例的实际游戏端口。
- 总览左侧控制区缩为约三成，按钮宽度相应缩小，右侧地址区扩大。游戏库/响应式回归、production build、真实双世界及桌面/窄桌面/手机检查通过；后续候选抽验端口读取恢复与 IPv6。

## 2026-09-17：页面背景拼接竖线（本地完成，未发布）

- 已修正共用木框的内侧纸纹拼接，玩家、存档和服务器健康页卡片间隙的细竖线消失；背景层不影响交互。
- 响应式回归链、production build、真实页面导航及桌面/窄桌面截图验收通过；后续候选抽验背景拼接与四角衔接。

## 2026-09-17：总览布局与中文任务提示（本地完成，未发布）

- 总览保留金黄色纸面、棕色边框和头像样式；存档数量位于顶部农场信息旁，控制/连接区下方为左侧在线玩家、右上近期事件、右下模组状态。面板标题和玩家底部入口以透明背景、细分隔线融入纸面，原文字/按钮/位置保留，支持窄容器重排。
- 任务名称、状态、常见错误、开发版标签及命名农场后缀已汉化；失败事件可打开中文详情并跳转对应任务，原始诊断折叠保留。旧生命周期记录不推测操作，未知模组更新结果明确待检查。
- 前端专项/响应式/命令结果/生命周期回归与 production build 通过；浏览器验证真实详情跳转、设置与确认取消，以及 1280/900/390 宽度页面。后续发布按候选流程验收，当前未发布。实现和交接见前端长期文档同日章节。

## 2026-09-17：右侧栏面板响应（本地完成，未发布）

- 2026-09-18 已调整：`<200ms` 绿、`200–399ms` 蓝、`400–999ms` 黄、`≥1000ms` 红，指示灯与显示数值一致；阈值及舍入边界回归、四档浏览器样式检查和 production build 通过。
- 已接入当前浏览器到 Panel 的 HTTP 响应时间，复用轻量版本接口、5 秒采样间隔、3 秒超时和说明浮层；未知/失败灰态、正常与慢响应按阈值着色，支持自动重试与隐藏/卸载清理。
- 前端专项/响应式回归、production build、Chrome 实际读数和合成超时/失败恢复/高延迟状态通过。后续候选抽验远程代理链路和真实页面后台切换；当前保留本地未发布。

## 2026-09-17：世界卡片比例优化（本地完成，未发布）

- 世界卡片采用紧凑布局：桌面实测 240×280px，96px 封面完整展示农场图，名称与状态分行，地址/按钮底部对齐。修改 `GameLibrary.css` 和既有响应式断言，接口与生命周期不变。
- 响应式、游戏库状态回归与 production build 通过；真实双世界日夜主题及改名打开/取消、390/320px 与手机横屏长名称夹具验收通过，无页面横向溢出或控制台错误。保留本地待视觉确认；后续发布抽验不同地图与长地址。

## 2026-09-17：玩家与诊断页面背景边缘（本地完成，未发布）

- 两页根背景透明，复用外框纸纹，顶部木框阶梯内角完整；改动仅涉及 `PlayersPage.css` / `DiagnosticsPage.css` 的背景层。
- 响应式回归、production build 和 CUA Chrome 1518×989 真实页面检查通过，页面切换正常、无横向溢出或控制台错误。保留本地，后续发布候选抽验背景与边框衔接。

## 2026-09-17：三级资源统计与首页位置（本地完成，未发布）

- 世界资源条双色比较已完成：右侧栏和诊断区域以紫灰底层表示整机总占用、原色前层表示世界占用，两层同起点且整机长度包含世界。小图例、悬浮明细、缺失与停服边界同步；前端回归/构建及两处实际填充长度、七种状态、390px 浏览器验证通过。保持本地待验收。
- 三处资源展示细化已完成：整机小圆环保留当前百分比，游戏卡片/世界面板显示「自身 / 整机当前占用」，悬浮显示核心当量或具体容量。共用防裁切浮层，支持点击、聚焦、Escape、外部关闭和首页横屏方向；标题/安装状态同排，正方形与分界线保持。世界接口补齐独立 `machine` 比较样本，趋势仍只反映当前世界。0/未知/100%/<0.1%、1518/390/320px、三处数值与交互、前端回归/构建、后端接口专项/vet/build 均通过；本地待视觉验收，8090 需保留原配置重启加载新字段。
- 已实现：首页顶栏三个整机小圆环，游戏正方形卡片内三项汇总，世界既有面板显示本世界指标与趋势。
- 已实现：系统 CPU/内存采样、按机器容量归一化、游戏容器与共享存储去重、世界实际文件/卷容量、失败与停止状态区分、缓存/并发限制。
- 已验证：前端构建与相关状态回归、桌面/窄屏浏览器布局、后端专项和真实 Docker 两世界接口/停止隔离；测试资源已清理。
- 接手：8090 原后端保留原终端配置重启后与前端联调；当前仅本地代码，尚未提交或发布。存储为文件和数据卷容量，机器磁盘为数据所在文件系统；不将 Docker 镜像层或多磁盘总量混入这些读数。
...
- `ResourceMonitor.tsx/.css` 接管资源指标和趋势图：纵向大数字、20 段横条、统一三色、容量说明与高占用提示。处理读取中、未启动、首次失败、采样中断、零占用及多核 CPU；CPU 保留超过 100% 的真实值。没有新增 API 或后台采样。
- `resource-metrics-presentation.ts` 保持最近 24 次采样、8 秒轮询；按时间排序去重并对缺失/超过 24 秒间断断线，页面准确标为「最近采样」。窄屏图表按容器尺寸排版，保持刻度可读。
- 新增 `test:resource-metrics` 并接入响应式回归；相关回归及 production build 通过，Browser 320px 状态夹具与 Chrome 实际停服诊断页通过。`qa-layout.html?resourceQa=preview` 覆盖八种状态；未执行游戏启停。后续正式候选抽验真实运行采样及恢复，当前保留本地未提交/未发布。
...
## 2026-09-15：移动控制页快捷操作排序（已实现，本地待验收）

- 七项快捷操作支持 450ms 长按换位、边缘滚动和松手自动保存，标题右侧显示操作提示；用户/世界顺序仅保存在当前浏览器。短按/滑动、按钮点击、多指/取消、键盘排序和原有权限均有独立处理。
- 文件：`SortableControlActions.tsx`、`control-action-order.ts`、`MobileControlPage.tsx/.css`、`StardewMobileShell.tsx`；新增状态测试与 `qa-control-action-order.ts` 浏览器合成触摸夹具。排序测试由既有响应式回归串联，production build 与合成触摸交互回归通过；接口无变化。
- 320×698 / 364px 窄屏标题提示同一行，无横向溢出；重新打开保留顺序、语言设置弹窗开关通过。未提交、未推送、未发布；下一步在 iOS/Android 真机验收触摸手感，合成触摸不替代真机验证。
...
- [x] 正式候选 `33177568325`（`29m25s`）完成 Panel fresh/restart、`v0.6.0 → v0.6.1` unhealthy 回滚与 healthy Web apply、immutable DLL/manifest/hash 和 required gate 契约复验，以及 `v0.3.2 → v0.6.1` 最老受影响边界直升；真实 `!login` fan-out 只归用户实机证据。
...
- [x] 最终身份为 `v0.6.1@5c0135e6bdb5b8353d049030da3c3c06a6e243a0`；三项条件门禁全部 selected、conditional skipped=无；proof artifact=`release-candidate-0.6.1-5c0135e6bdb5`（ID `9689244375`），自动 annotated tag=`33179940424`（`19s`）、正式提升=`33179959858`（`1m26s`），三仓 `0.6.1/latest` 六引用统一 digest=`sha256:9b7746caeb9c3c9091e7e6c07b1cc3cdf18dce82ebcfb5a70bc4c5a5835961bd`。
- [x] 两条升级 fixture 的任务 owner/container/network/volume 强制清零门禁通过；外层候选与正式 smoke 使用精确名称的 best-effort EXIT trap，并由 hosted runner 终止回收，没有使用生产数据、长期凭据或 prune。正式 promotion 只提升候选 proof 中的精确 digest，未重新构建镜像。
- [ ] 发布后证据审计尚不能证明用户双客户端实机是在“通过 Web 升级得到的新 Panel”上执行；候选只证明同一 immutable Control 制品、required gate 与升级边界，fixture 没有发送真实聊天。若用户确认本次实测就是该上下文，则据实补齐；否则下一版门禁必须把升级后真实 `!login` fan-out 验收绑定到同一条证据。
