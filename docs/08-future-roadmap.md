...
- [x] Control-only 仅在 server/auth/JunimoServer Mod 均未变化时允许一次默认 2 秒 `/health` advisory；容器 running 与精确 image ID 保持硬门禁，容器内 1 秒 watchdog 保证探针有界，最终验证不重复探测，`/steam/ready` 始终禁用。
