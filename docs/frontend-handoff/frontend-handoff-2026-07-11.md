...
- 按钮外观：`OverviewPage.tsx/.css` 使用 `sd-btn-green` 绿色木纹像素边框、浅色文字，尺寸为最小宽 88px、高 32px，复用统一交互状态。production build、Chrome 1473px/900px 截图与审批/权限边界复验通过；后续保持与绿色主按钮皮肤一致。

- 改动：`OverviewPage.tsx/.css` 把在线且未认证的非 host 玩家排在 host 后面，显示「待批准」和黄色状态点，右侧增加「批准」。所有待批准玩家优先展示，不受默认四行预览截断。
- 接口：沿用当前实例的密码状态、批准认证 API 与命令结果轮询；管理员、运行状态、密码认证、认证桥、联机 ID 全部满足才可批准，确认框保留，处理中禁止重复提交，成功后刷新列表。未改变后端接口。
- 验证：命令结果/响应式回归链、production build 通过；Playwright/Chrome 在隔离夹具完成 1473×1033 和 900×1033 排序及外观验收，以及取消、成功刷新、失败保留、多待批准、权限和缺失能力边界，控制台无警告/错误、无横向溢出。截图位于本轮系统临时目录，未加入仓库。
- 下一步：本地未发布；正式候选使用真实 Junimo 待认证玩家抽验批准闭环，本轮仅模拟提交与结果。

## 2026-09-17：玩家活动随认证状态排布（本地完成，未发布）

- 改动：`PlayersPage.css` 使用两个等宽网格列，活动卡片自动排布。未开启密码认证时位于左列；开启时由待认证区域占据左列，活动卡片位于同排右列，两卡等宽等高。两卡使用 `align-self: stretch` 和自动高度跟随网格行，事件区保留最小高度；单列时各自按内容撑开。继续复用 `PlayersPage.tsx` 已有的认证条件和事件分页，无接口变化。
- 验证：`test:responsive-layout` 回归链及 production build 通过；Chrome 1473×1033 真实双世界确认两种布局，下一页从 1 / 25 更新到 2 / 25；900×900 单列正常，无横向溢出和控制台警告/错误。
- 等高验收：Chrome 1473×977 两卡高度均为 249.97px，上下边缘相同，下一页正常；900×900 保持单列自适应，响应式回归链与 production build 再次通过。
- 下一步：本地未发布；候选抽验认证状态变化、活动分页与窄容器重排。

## 2026-09-17：服务器控制与消息布局（本地完成，未发布）

- 改动：`ServerControlPage.css` 顶部约 4:6 分配生命周期和消息区域，快捷操作在下方按最小 240px 自适应横排；窄容器恢复纵向顺序。
- 状态：`ServerControlPage.tsx` 运行态只呈现停止/重启，可启动状态显示启动，过渡状态保留加载反馈。沿用生命周期 hook、权限、二次确认和消息接口。
- 验证：生命周期状态回归、响应式回归链、production build 通过。Chrome 1473×977 真实运行态确认停止/重启、消息框同排和三列快捷按钮；停止确认打开/取消、语言弹窗打开/关闭、消息 4/120 与清空禁用通过。900×900 停止态夹具仅显示启动且两列快捷操作；390×844 真实移动控制页正常。无横向溢出及控制台警告/错误，未执行真实启停或发送消息。后续候选抽验长消息与过渡状态。

## 2026-09-17：直连地址端口与总览控制区（本地完成，未发布）

- 改动：新增 `connection-address.ts`，统一世界卡、`LanDirectConnectCard.tsx`、`MobileHomePage.tsx` 的展示与复制。24642 省略端口，其它端口保留；IPv6、空主机和非法端口有明确处理。
- 数据：`useStardewDashboardData.ts` 从当前实例 `/public-ip` 取得游戏端口，与面板访问主机组合；请求独立加载，并用代次阻止旧请求/卸载回写。没有新增后端接口。
- 布局：`OverviewPage.css` 左右列约 3:7，按钮最大 140px，给右侧地址更多宽度，窄容器沿用上下排列。
- 验证：`test:game-library`、`test:responsive-layout` 和 production build 通过；Chrome 真实 24642/24643 双世界显示、复制、同步和切换通过，1473/900/390px 无横向溢出和控制台警告/错误。新增回归位于 `test-game-library-state.ts`。
- 下一步：本地未发布；候选抽验多实例自定义端口、慢连接失败/同步恢复和 IPv6 地址。

## 2026-09-17：页面背景拼接竖线（本地完成，未发布）

- 改动：`StardewPanel.css` 为共用主框增加居中纸纹层，与边缘贴图重叠 2px；内容位于纸纹上方，纸纹不拦截交互。玩家、存档和服务器健康页卡片间隙的细竖线已消除，无接口变化。
- 验证：响应式回归链和 production build 通过；Chrome localhost:5173 真实页面导航、1473×1033 三页截图、900×900 诊断页截图通过，木框完整、无横向溢出，控制台无警告/错误。
- 后续：调整主框边缘厚度时检查纸纹重叠及四角衔接；本次为本地样式修正，未发布。
...
- 修改 `GameLibrary.css` 与既有 `test-responsive-layout.ts` 断言：桌面卡片宽 224–240px、最小高 280px，真实 1518×989 下为 240×280px；封面 96px，紧凑模式 90px，农场图完整缩放。名称独占一行、超长省略，状态位于下一行，地址及操作底部对齐。
- 继续使用卡片原有完整名称 title/accessible label、改名表单、复制和生命周期回调；44px 复制/启停操作区保持。编辑状态可自然增高，取消或 Esc 后回到等高布局。没有接口、数据与权限变化。
- 验证：响应式回归、游戏库状态回归、production build 通过；最终尺寸再次通过响应式回归、构建及真实 Chrome 240×280px 和英文名称单行检查。真实双世界日夜主题与改名打开/取消通过；CUA 应用内 Browser 既有混合长名称夹具在 390×844、320×698、844×390 下无页面横向溢出、长名称不撑高卡片、Esc 退出正常，控制台无警告/错误。竖屏沿用既有横向旋转壳。
- 下一步：本地视觉确认，正式候选抽验不同农场图、长地址与改名错误态。未执行真实游戏启停/删除或提交改名；未提交、未推送、未发布。
...
- `MobileControlPage` 的七张快捷卡片接入 `SortableControlActions.tsx`：长按 450ms 拿起、上下换序、边缘自动滚动、松手持久保存。提示放在「快捷操作」标题右侧，保持原卡片与 64×44px 按钮布局；操作按钮不参与拖动，原权限和确认链路不变。
- `control-action-order.ts` 归一化卡片 ID 并管理可取消的延时手势；按用户/实例隔离 localStorage，版本号为 1，坏数据回退、未知项过滤、缺失项追加。`StardewMobileShell` 向控制页传 `instanceId`，排序组件以存储 key 为 React key，切换用户/世界重新读取偏好。没有新增 API。
- 长按前移动超过 8px 或开始滚动即取消；激活后阻止触摸滚动并用 RAF 做边缘滚动。多指、touchcancel、Escape、blur、隐藏页面回退；卸载清理监听、计时器和 RAF。排序松手抑制尾随点击，Alt+上下键提供键盘入口，屏幕阅读器播报结果。
- `test:control-action-order` 覆盖读写、隔离、顺序边界、旧回调、误触与取消，由 `test:responsive-layout` 串联；两项回归和 production build 通过。QA 入口：`qa-layout.html?surface=app&route=/instances/stardew/overview?shell=mobile&state=running&controlQa=gestures`，进入「控制」后点「运行快捷排序回归」。合成触摸对真实组件的短按、滑动、按钮排除、长按移动、多种取消、边缘滚动、保存、防误点与键盘换序全部通过，并恢复测试前顺序。
- 视觉/交互补充：320×698 和 364px 下提示与标题同一行、无横向溢出，按钮保持 64×44px；完整导航后顺序仍保留，换序后的语言设置弹窗开关正常。完整导航后无新增控制台错误；QA 入口热更新曾产生两条 createRoot 历史告警，已记录错题本。右侧预览恢复普通夹具并保留原排列。
- 下次注意：QA 入口被 SPA 改写后重载需重新输入完整夹具 URL；页面内回归为合成触摸，iOS/Android 真机手感仍待验收。用户要求所有改动保留本地，不提交、不推送、不发布。
