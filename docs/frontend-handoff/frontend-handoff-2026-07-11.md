...
- 素材由内置 imagegen 以原 `button_secondary_small_tan_blank.png` 为参考生成，文件保存在 `frontend/public/assets/stardew/ui/buttons/button_frame_warm_9slice.png`。提示词约束：single square nine-slice source tile, frame reaches canvas edges, thin warm walnut pixel outline, honey wood bevel, cream top-left highlight, flat parchment center, tiny stepped corners, no text or icons。实际接入只使用外围 10% 边框、不绘制图片中心；文字、图标和语义底色独立渲染。
