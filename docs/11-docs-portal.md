...
状态：**步骤 1-8 的既有正式官网继续上线；2026-07-29 的任务型隔离改版未经发布授权，2026-08-07 误合并后已撤回。当前官网保留原 Hero、联机邀请卡、六入口和原导航并展示 v0.4.10；2026-08-10 首页 QQ 群沟通入口及顶栏/首屏响应式修正已发布，桌面首屏两段纵向留白的二次收紧已在本地完成、尚未发布**。以下决策已和用户对齐：
...
- 发布后二次留白跟进（本地完成，未发布）：仅在 `>=960px` 把 Hero 顶/底 padding 从 `nav + 26px / 70px` 收到 `nav + 10px / 50px`，并给 `.VPHomeFeatures` 增加 `margin-top:-12px`。1700×1100 下顶栏到品牌行由约 72.36px 收到 48px，加群卡到首张功能卡由约 87.64px 收到 64px；390/959px 原规则不变，960/1024/1700px 无溢出或 overlay，959px 菜单正常。
