...
assert.ok(/isAdmin \? <button[^>]*onClick=\{\(\) => \{ void openRuntimeSettings\(\) \}\}[^>]*aria-label="修改联机人数上限">修改上限<\/button> : null/.test(overview), 'overview player limit action remains admin-only and opens shared settings')
