...
const installDownloadIcon = readFileSync(new URL('../public/assets/stardew/ui/install/icon_install_step_download_image2_regen.png', import.meta.url))
assert.equal(installDownloadIcon.subarray(0, 8).toString('hex'), '89504e470d0a1a0a', 'download icon must be PNG')
assert.equal(installDownloadIcon.readUInt32BE(16), 72, 'download icon width')
assert.equal(installDownloadIcon.readUInt32BE(20), 72, 'download icon height')
assert.equal(installDownloadIcon[25], 6, 'download icon must keep RGBA transparency')
...
const authCardCss = readTextFile(new URL('../src/core/AuthCard.css', import.meta.url))
...
assert.match(authCardCss, /\.sd-auth-shell\.sd-auth-shell--compact\s*\{[^}]*min-height:\s*100dvh/s)
assert.match(authCardCss, /\.sd-auth-card\s*\{[^}]*width:\s*min\(360px,\s*100%\)/s)
...
assert.match(gameLibrarySource, /getInstanceDirectConnect\(instanceId\)/)
...
assert.doesNotMatch(targetedWorldRefresh[0], /getInstanceDirectConnect/)
...
assert.match(gameLibraryCss, /\.game-hub:not\(\.game-hub--background-night\) \.game-carousel-card\s*\{[^}]*background:\s*var\(--sd-card-bg\)/s)
assert.match(gameLibraryCss, /\.game-hub:not\(\.game-hub--background-night\) \.world-choice\s*\{[^}]*background:\s*var\(--sd-card-bg\)/s)
assert.match(gameLibraryCss, /\.game-hub:not\(\.game-hub--background-night\) \.world-create-card\s*\{[^}]*background:[^}]*var\(--sd-card-bg\)/s)
...
assert.match(stardewRoutesSource, /refreshPublicIP: \(\) => Promise<void>/)
