...
const errorCodeMap: Record<string, string> = {
  ...
  save_name_encoding_invalid: '存档目录名编码异常，请备份或删除后使用 UTF-8 ZIP 重新上传',
  select_failed: '选择存档失败',
  save_delete_failed: '删除存档失败，原存档仍会保留或可从删除前备份恢复',
  delete_failed: '删除存档失败',
  export_failed: '导出存档失败',
  import_failed: '导入存档失败',
  invalid_zip: '存档 ZIP 无效',
  invalid_config: '配置参数无效',
  farm_type_not_installed: '未找到该模组农场，请确认 FarmType Id 与已安装 Mod。',
  farm_dependencies_missing: '模组农场缺少必需依赖，请先安装并准备依赖。',
  farm_type_not_loaded: '本次启动没有加载目标农场，未发送创建请求。',
  farm_catalog_stale: '运行时农场目录已过期或不属于本次创建事务。',
  runtime_catalog_stale: '运行时农场目录已过期或不属于本次创建事务。',
  farm_type_mismatch: '实际存档 FarmType 与请求不一致，错误存档未激活并已恢复原状态。',
  new_game_result_unknown: '新建结果未知，系统不会自动重试，请检查任务详情。',
  new_game_outcome_unknown: '新建结果未知，系统不会自动重试，请检查任务详情。',
  new_game_rollback_failed: '新建失败且未能完整恢复，请停止操作并检查任务日志。',
  mod_profile_commit_failed: '存档已创建，但 Mod 配置提交失败；存档已保留，需要重试配置提交。',
  invalid_body: '请求格式错误',
  ...
  import_recovery_required: '导入需要人工恢复，请勿再次提交或重启服务器',
  import_activation_timeout: '目标存档未能在期限内完成加载',
  save_in_progress: '服务器正在保存、运行或接管该上传事务，请稍候',
  // Nexus
