...
export function SetupPanel({
  ...
  return (
    <form className="form-grid" onSubmit={onSubmit} autoComplete="off">
      ...
      <Field label="管理员密码">
        <PasswordInput value={form.password} visible={showPwd} placeholder=" " autoComplete="new-password"
          onChange={(p) => onChange({ ...form, password: p })} onToggle={() => setShowPwd((v) => !v)} />
      </Field>
      <Field label="确认密码">
        <PasswordInput value={form.confirmPassword} visible={showConfirm} placeholder=" " autoComplete="new-password"
