  ...
  return (
    <div className="sd-page">
      ...
      <div className="sd-page-header">
        ...
        />
        <div>
          <h2 className="sd-page-title">首次安装向导</h2>
          <p className="sd-page-desc">配置 Steam 凭据并安装 Stardew Valley 服务器（含 SMAPI + JunimoServer）。</p>
        </div>
      </div>
      ...
      <div className="sd-state-card">
        ...
        {stateMessage ? (
          <div className="sd-state-row">
            ...
            <span className="sd-install-state-msg">{stateMessage}</span>
          </div>
        ) : null}
      </div>
      ...
      {/* ── 安装进度区 ──────────────────────────────────────────────────── */}
      {showProgress ? (
        <div className="sd-install-progress-section">
          <div className="sd-install-section-title">安装进度</div>

          {/* 步骤条 */}
          <ol className="sd-install-steps">
            {STEPS.map((label, i) => (
      ...
      {(needsAuthMethodChoice || needsGuardChoice || needsGuard || needsQrCode) && !isAdmin ? (
        <div className="sd-install-guard-section">
          <div className="sd-install-section-title">Steam 认证</div>
