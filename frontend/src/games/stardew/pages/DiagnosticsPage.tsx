...
function junimoUserPhase(dryRun: JunimoUpdateDryRunStatus | null, apply: JunimoUpdateApplyStatus | null) {
  if (apply && apply.phase !== 'idle' && !preferDryRunWorkflow(dryRun, apply)) {
    ...
    const tone: NonNullable<UserUpdateProgressProps['tone']> = apply.phase === 'succeeded' ? 'success' : apply.phase === 'failed_rolled_back' ? 'warning' : apply.phase === 'rollback_failed' ? 'error' : 'active'
  ...
  return { headline: junimoDryRunPhaseLabel(dryRun?.phase), progress: (dryRun?.progress ?? 0) * 0.25, currentStep, tone, error: dryRun?.error, download: dryRun?.download }
}
...
function smapiUserPhase(dryRun: SMAPIUpdateWorkflowStatus | null, apply: SMAPIUpdateWorkflowStatus | null) {
  if (apply && apply.phase !== 'idle' && !preferDryRunWorkflow(dryRun, apply)) {
    ...
    const currentStep = apply.phase === 'checking' ? 0 : downloadPhases.includes(apply.phase) ? 1 : installPhases.includes(apply.phase) ? 2 : verifyPhases.includes(apply.phase) ? 3 : 0
    const tone: NonNullable<UserUpdateProgressProps['tone']> = apply.phase === 'succeeded' ? 'success' : apply.phase === 'failed_rolled_back' ? 'warning' : apply.phase === 'rollback_failed' || apply.phase === 'failed' ? 'error' : 'active'
    return { headline: smapiPhaseLabel(apply.phase), progress: 25 + apply.progress * 0.75, currentStep, tone, error: apply.error }
  }
  const tone: NonNullable<UserUpdateProgressProps['tone']> = dryRun?.phase === 'failed' ? 'error' : 'active'
...
export function DiagnosticsPage({ user, dashboardData, instanceState }: StardewPageProps) {
  ...
  const junimoRepairPlan = junimoUpdate?.repairPlan
  const junimoRepairAvailable = junimoRepairPlan?.actionAvailable === true
  ...
  const junimoNeedsAttention = !!junimoRepairPlan
    || junimoMaintenanceNeedsAttention(junimoStatus, junimoUpdateAvailable, junimoApply?.phase, junimoUpdateError !== null || junimoApplyError !== null)
  const junimoNeedsReview = (!!junimoRepairPlan && !junimoRepairAvailable)
    || (junimoNeedsAttention && !junimoUpdateAvailable && !junimoRepairAvailable && !junimoApplyActive(junimoApply?.phase))
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      getJunimoUpdateApply().then((result) => {
        setJunimoApply(result)
        setJunimoApplyError(null)
        if (!junimoApplyActive(result.phase)) void getJunimoUpdate().then(setJunimoUpdate)
      }).catch((e) => setJunimoApplyError(errorMessage(e)))
    }, 1800)
    return () => window.clearInterval(timer)
  ...
  async function handleJunimoUpgrade() {
    if (!junimoUpdate || !junimoUpdateAvailable || junimoRepairPlan || junimoApply?.phase === 'rollback_failed') return
    const runningNote = junimoDryRun?.serverRunning || instanceState?.state === 'running'
  ...
  async function handleJunimoRepair() {
    if (!junimoRepairPlan?.actionAvailable || junimoUpgradeBusy) return
    const steps = junimoRepairPlan.steps.map((step, index) => `${index + 1}. ${step}`).join('\n')
    if (!window.confirm(`${junimoRepairPlan.buttonLabel}？\n\n检测依据：${junimoRepairPlan.detection}\n\n修复方法：${junimoRepairPlan.method}\n\n执行步骤：\n${steps}\n\n后端会在执行前重新检测；条件变化、未知问题或复检失败都会停止并保留现场。`)) return
    setJunimoApplyBusy(true)
    setJunimoApplyError(null)
    try {
    ...
    <div className="sd-page sd-diag-page">
      ...
      {isAdmin ? (
        <section id="runtime-components-update" className="sd-card sd-diag-source-panel sd-diag-junimo-panel" aria-label="游戏运行文件版本">
          ...
          <div className="sd-diag-dry-run" aria-label="游戏运行文件只读预检">
            <div className="sd-diag-dry-run-actions"><span>用户操作入口位于上方“版本维护”卡片；这里仅保留原始检查结果。</span></div>
      ...
      <section id="junimo-update" className="sd-card sd-diag-source-panel sd-diag-junimo-panel" aria-label="Junimo 运行组件版本对">
        ...
        {isAdmin ? (
          <div className="sd-diag-dry-run" aria-label="Junimo 运行组件升级预检">
            <div className="sd-diag-dry-run-actions"><span>用户操作入口位于上方“版本维护”卡片；这里仅保留镜像、digest、检查项和脱敏日志。</span></div>
      ...
      <section className="sd-diag-maintenance-panel" aria-label="版本维护建议">
        ...
        <div className="sd-diag-maintenance-list">
          {junimoNeedsAttention ? (
            <div className="sd-diag-maintenance-item">
              <div className="sd-diag-maintenance-item-main">
                <span className="sd-diag-maintenance-item-icon" aria-hidden="true">{junimoRepairPlan ? '!' : '↑'}</span>
                <div className="sd-diag-maintenance-copy">
                  <strong>{junimoRepairPlan?.title || (junimoNeedsReview ? 'Junimo 版本状态需要检查' : 'Junimo 服务组件有推荐更新')}</strong>
                  <p>{junimoRepairPlan
                    ? `检测：${junimoRepairPlan.detection} 处理：${junimoRepairPlan.method}`
                    : junimoNeedsReview
                ...
                {isAdmin ? (junimoRepairAvailable
                  ? <button className="sd-btn-green sd-btn--sm" type="button" disabled={junimoUpgradeBusy} onClick={handleJunimoRepair}>{junimoUpgradeBusy ? '修复进行中…' : junimoRepairPlan.buttonLabel}</button>
                  : junimoRepairPlan?.action === 'export'
                    ? <button className="sd-btn-green sd-btn--sm" type="button" disabled={exportBusy} onClick={handleExportBundle}>{exportBusy ? '导出中…' : junimoRepairPlan.buttonLabel}</button>
                    : junimoRepairPlan?.action === 'wait'
                      ? <button className="sd-btn-green sd-btn--sm" type="button" disabled>{junimoRepairPlan.buttonLabel}</button>
                      : junimoNeedsReview
                        ? null
