...
- 面板默认使用局域网 / IP 直连，不要求服务器预先运行 SteamAuth。需要 Steam 邀请码时，可在安装完成后由管理员按需启用并单独完成授权。
...
2. 在弹出的配置面板里：
   ...
   试用页面会提示"当前试用 ECS 为按量付费实例，不满足国内 ICP 备案要求"——这个提示只影响想用域名对外提供网站访问的场景。星露谷服务器默认走 IP 直连，也可以由管理员按需启用 Steam 邀请码加入；这两种游戏连接方式都不需要备案，可以忽略这条提示。
   :::
