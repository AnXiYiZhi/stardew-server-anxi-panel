...
GitHub 只读状态/制品查询遭遇 EOF 等瞬时网络错误时，使用同一 run ID、artifact 名或精确引用最多三次有界重读；监控失败不代表 workflow 失败。不得把只读重试用于 dispatch、取消、tag、push、Release edit 或镜像提升；写操作响应不确定时先核对外部实际状态。
